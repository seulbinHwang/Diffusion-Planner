...
def _load_model_and_ema_state_only_from_deepspeed_checkpoint(
    ...
    mp_rank_str = sorted(candidates)[0]
    ...
    # ✅ 요구사항 반영:
    # - preproj.fc1.weight 4ch->6ch 확장/복사 로직 제거
    # - shape mismatch면 여기서 RuntimeError로 그대로 실패(키 삭제로 회피하지 않음)
    ...
    if global_rank == 0:
        ...
        # 기존 strict=True 기대 동작을 유지(=키 불일치는 에러)
        if missing_keys or unexpected_keys:
            raise RuntimeError(
                f"[ModelOnly<DeepSpeed>] 모델 파라미터 불일치가 발생했습니다. "
