...
def draw_neighbor_future_gt_3_dim(
    ...
    """이웃 차량의 GT 미래 궤적(토큰별 [x, y, yaw] 시퀀스)을 그립니다.

    변경된 부분(고속화)
    ------------------
    draw_mode="line"일 때,
    - 기존: 선분마다 ax.plot 호출
    - 변경: 유효한 점들을 뽑아 (K,2,2)로 만든 뒤 LineCollection 1개로 그림

    다른 모드(point/arrow)는 기존과 동일하게 유지합니다.
    """
    eps: float = float(options.invalid_eps)
    draw_mode: str = str(options.DIFF_future_gt_3_dim_draw_mode)
    ...
    for track_token, future_gt_3_dim in diff_token_to_future_gt_3_dim.items():
        ...
        future_gt_3_dim = np.asarray(future_gt_3_dim)  # shape: (T,3)
        ...
        if draw_mode == "line":
            ...
            if valid_xy.shape[0] >= 2:
                ...
                _add_line_collection(
                    ...
                    colors=options.DIFF_future_gt_3_dim_COLOR,
                    line_width=float(options.DIFF_future_gt_3_dim_line_width),
                    linestyle="-",
                    zorder=26,
                )
            # 라벨 규칙은 기존 그대로: 첫 프레임(t==0)이 유효할 때만
            if options.DIFF_draw_diff_future_gt_3_dim_token and future_len > 0:
                first: Array = future_gt_3_dim[0]  # (3,)
        ...
        # ───────── point / arrow 모드(기존 유지) ─────────
        prev_xy: Optional[Tuple[float, float]] = None
        for t in range(future_len):
            row: Array = future_gt_3_dim[t]  # (3,)
            ...
            # (point/arrow에서도 prev_xy는 쓰지 않음. 기존 로직 그대로)
            prev_xy = (x, y)

        if options.DIFF_draw_diff_future_gt_3_dim_token and future_len > 0:
            first: Array = future_gt_3_dim[0]
            if is_valid_future_row_xyyaw(first, eps):
                fx: float = float(first[0])
                fy: float = float(first[1])
                ax.text(
...
# =============================================================================
def _is_dashed_linestyle(linestyle: Any) -> bool:
    """linestyle이 점선 계열인지 판단합니다.

    Matplotlib에서는 점선을 여러 방식으로 표현합니다.
    - 문자열: "--", "-.", ":" 등
    - 튜플: (offset, (on_off_on_off...)) 형태

    이 함수는 입력이 점선이면 True, 실선이면 False를 반환합니다.

    Args:
        linestyle (Any):
            Matplotlib에서 받는 선 스타일 값. shape: ()

    Returns:
        bool:
            점선이면 True, 실선이면 False. shape: ()
    """
...
def _add_line_collection(
    ...
    """여러 선분을 한 번에 그립니다(LineCollection 사용).

    기존 구현은 선분 1개마다 ax.plot을 호출해서 선분 개수만큼 그리기 객체가 생깁니다.
    차선처럼 선분이 수만 개가 되면, 그 객체를 만들고 관리하는 비용이 매우 커집니다.

    이 함수는 (N, 2, 2) 선분 배열을 받아 LineCollection 1개로 추가합니다.
    - cap/join/antialias/snap을 rcParams와 동일하게 맞춰서
      ax.plot 결과와 시각적으로 완전히 같게 유지합니다.

    Args:
        ax (plt.Axes):
            Matplotlib 축 객체. shape: ()
        segments_xy (np.ndarray):
            shape = (N, 2, 2)
            N개의 선분.
            segments_xy[i, 0] = [x0, y0]
            segments_xy[i, 1] = [x1, y1]
        colors (Any):
            - str이면 모든 선분이 같은 색
            - 길이 N인 리스트/배열이면 선분별 색 다르게 지정 가능. shape: (N,) 또는 ()
        line_width (float):
            선 두께. shape: ()
        linestyle (Any):
            선 스타일. 예) "-", "--", (0, (4, 4)) 등. shape: ()
        zorder (int):
            그리기 순서. shape: ()
    ...
    Returns:
        None
    """
    seg: Array = np.asarray(segments_xy)
    if seg.size == 0:
        return
    ...
    capstyle, joinstyle = _get_cap_and_join_style_for_linestyle(linestyle)
    line_collection.set_capstyle(capstyle)
    line_collection.set_joinstyle(joinstyle)
...
def _build_line_segments_from_points_xy(
    points_xy: Array,
    segment_drawable: Array,
) -> Array:
    """점 배열과 선분 유효 마스크로 (K, 2, 2) 선분 배열을 만듭니다.

    Args:
        points_xy (np.ndarray):
            shape = (T, 2)
        segment_drawable (np.ndarray):
            shape = (T-1,)
            True인 i에 대해 (i -> i+1) 선분을 만듭니다.
    ...
    if seg_ok.ndim != 1 or seg_ok.shape[0] != max(int(pts.shape[0]) - 1, 0):
        raise ValueError(
            f"segment_drawable shape는 (T-1,) 이어야 합니다. got {seg_ok.shape}, T={pts.shape[0]}"
        )

    if seg_ok.size == 0 or not np.any(seg_ok):
        return np.zeros((0, 2, 2), dtype=pts.dtype)

    idx: Array = np.nonzero(seg_ok)[0]  # shape: (K,)
    p0: Array = pts[idx]  # shape: (K, 2)
    p1: Array = pts[idx + 1]  # shape: (K, 2)
    return np.stack([p0, p1], axis=1)  # shape: (K, 2, 2)


def _build_double_line_segments_from_center_and_boundary_xy(
    center_xy: Array,
    boundary_xy: Array,
    segment_drawable: Array,
    sep_m: float,
) -> Tuple[Array, Array]:
    """이중선(두 줄) 선분 배열(안쪽/바깥쪽)을 한 번에 계산합니다.

    각 선분마다 경계선의 방향에 수직인 방향을 만든 뒤,
    "차선 중심 쪽"을 안쪽으로 보고 그 방향으로 조금 밀어서(shift)
    두 줄(안쪽/바깥쪽)을 만듭니다.
    ...
    Returns:
        Tuple[np.ndarray, np.ndarray]:
            inner_segments, outer_segments
            - inner_segments: shape = (K, 2, 2)
            - outer_segments: shape = (K, 2, 2)
    """
    ...
    if seg_ok.size == 0 or not np.any(seg_ok):
        empty: Array = np.zeros((0, 2, 2), dtype=np.float64)
        return empty, empty

    idx: Array = np.nonzero(seg_ok)[0]  # shape: (K,)

    c0: Array = cxy[idx]  # shape: (K, 2)
    c1: Array = cxy[idx + 1]  # shape: (K, 2)
    p0: Array = bxy[idx]  # shape: (K, 2)
    p1: Array = bxy[idx + 1]  # shape: (K, 2)

    seg: Array = p1 - p0  # shape: (K, 2)
    seg_len: Array = np.hypot(seg[:, 0], seg[:, 1])  # shape: (K,)

    normal: Array = np.zeros_like(seg)  # shape: (K, 2)
    safe_mask: Array = seg_len > 1e-6  # shape: (K,)
    normal[safe_mask, 0] = -seg[safe_mask, 1] / seg_len[safe_mask]
    normal[safe_mask, 1] = seg[safe_mask, 0] / seg_len[safe_mask]

    boundary_mid: Array = 0.5 * (p0 + p1)  # shape: (K, 2)
    ...
    shift: Array = normal * (0.5 * float(sep_m))  # shape: (K, 2)

    p0_in: Array = p0 + shift  # shape: (K, 2)
    p1_in: Array = p1 + shift  # shape: (K, 2)
    p0_out: Array = p0 - shift  # shape: (K, 2)
...
def _draw_lane_boundary_segments_with_plan(
    ...
    """경계선을 계획(plan)에 맞춰 선분 단위로 그립니다(고속 버전).

    핵심 변경점
    ----------
    - 기존: 선분 1개마다 ax.plot 호출 → 선분 수만큼 객체가 생김(매우 느림)
    - 변경: 모든 선분을 (K,2,2)로 모아서 LineCollection 1~2개로 그림(훨씬 빠름)

    출력이 완전히 같도록 보장하기 위해
    - 색/두께/linestyle/zorder를 기존과 동일하게 적용
    - cap/join/antialias/snap을 rcParams와 동일하게 맞춤

    단, 이중선에서 안쪽/바깥쪽 linestyle이 서로 다르면,
    선분 사이에서 겹침 순서가 아주 드물게 달라질 수 있어
    기존 방식(느린 경로)을 그대로 사용합니다.
    (PASSING_DOUBLE_YELLOW 같은 케이스)
    """
    if not plan.draw:
        return
    if plan.draw_mode != "line":
        return
    ...
    # (1) 이중선 + (안쪽/바깥쪽) 스타일이 다르면: 기존 방식(완전 동일한 순서 보장)
    if plan.is_double and (plan.inner_linestyle != plan.outer_linestyle):
    ...
    # (2) 고속 경로: LineCollection 사용
    if plan.is_double:
        inner_segments, outer_segments = _build_double_line_segments_from_center_and_boundary_xy(
            center_xy=cxy,  # (T,2)
            boundary_xy=bxy,  # (T,2)
            segment_drawable=seg_ok,  # (T-1,)
            sep_m=float(options.LANE_double_line_sep_m),
        )  # (K,2,2), (K,2,2)

        _add_line_collection(
            ax=ax,
            segments_xy=inner_segments,
            colors=plan.color,
            line_width=float(options.LANE_boundary_width),
        ...
        _add_line_collection(
            ax=ax,
            segments_xy=outer_segments,
            colors=plan.color,
            line_width=float(options.LANE_boundary_width),
            linestyle=plan.outer_linestyle,
            zorder=int(zorder),
        )
        return
...
def _draw_lane_boundary_highlight_segments(
    ...
    """경계선을 강조색으로 선분 단위로 그립니다(고속 버전).

    기존은 선분마다 ax.plot을 호출했지만,
...
def draw_lane_centerlines(
    ...
    센터라인 선 모드에서
    - 기존: 선분마다 ax.plot 호출
    - 변경: 선분들을 모아서 LineCollection 1개로 그림
    ...
    for lane_idx, lane_i in enumerate(lanes):
        ...
        # (1) 그릴 선분 마스크
        seg_ok: Array = valid_mask[:-1] & valid_mask[1:]  # (lane_len-1,)
        idx: Array = np.nonzero(seg_ok)[0]  # (K,)
        if idx.size > 0:
            p0: Array = center[idx]  # (K,2)
            p1: Array = center[idx + 1]  # (K,2)
            segments: Array = np.stack([p0, p1], axis=1)  # (K,2,2)

            # (2) 선분별 색(신호등 우선 규칙 유지)
            colors: List[str] = [
                _choose_lane_centerline_color(
                    signal_state_idx=int(signal_state_idx[int(j)]),
...
def draw_token_trajectory_non_rects(
    ...
    """(T,11) 궤적을 arrow/point/line 모드로 그립니다.
    ...
    반환(first_valid_xy) 규칙(기존과 동일)
    -----------------------------------
    - mode="line"은 "실제로 선분이 그려진 첫 지점"을 반환합니다.
      (즉, t와 t+1이 둘 다 유효한 첫 t)
    """
    if traj_11 is None or traj_11.size == 0:
        return None
    ...
    traj_xy: Array = traj[:, :2]  # shape: (T, 2)
    vx: Array = traj_11[:, 4]  # shape: (T,)
    vy: Array = traj_11[:, 5]  # shape: (T,)
    future_len: int = int(traj.shape[0])
    ...
    if mode == "line":
        ...
        _add_line_collection(
            ax=ax,
            segments_xy=segments,
            colors=style["line_color"],
            line_width=float(style["line_width"]),
            linestyle="-",
            zorder=int(zorder),
        )

        # first_valid_xy: "실제로 선분이 그려진 첫 지점"(기존 로직과 동일)
        first_valid_xy: Optional[Tuple[float, float]] = None
        if np.any(seg_ok):
...
def save_figure_to_png(fig: plt.Figure, save_path: str) -> None:
    """Figure를 PNG로 저장하고 Figure를 닫습니다."""
