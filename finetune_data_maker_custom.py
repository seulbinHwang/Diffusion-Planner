...
def _build_recovery_traj_raw_for_dataset_save(
    *,
    args: Any,
    unnorm_best_traj: torch.Tensor,  # (B, 1+Pnn, 1+future_len, 4)
    expert_distance_m: torch.Tensor,  # (B, 1+Pnn)
    unnorm_outputs_copy: Dict[str, torch.Tensor],
    target_future_gt_is_valid: torch.Tensor,  # (B, 1+Pnn, future_len)
    target_current_is_valid_expand: torch.Tensor,  # (B, 1+Pnn, 1+future_len)
    future_len: int,
) -> torch.Tensor:
    """저장용 미래 GT를 만들기 위해 recovery 궤적을 항상 계산합니다.

    핵심 동작(요청사항 반영)
    ----------------------
    - 이 함수는 "저장용 데이터"에만 사용합니다.
    - 실제 이동(execute)은 기존 로직을 그대로 사용합니다.
    - 저장용으로는 recovery_threshold_m 값을 무시하고,
      threshold를 0m로 가정해서 recovery 궤적을 계산합니다.
    - move_by_recovery=True로 데이터셋을 만들 때,
      저장용 계산이 항상 수행되도록 use_recovery도 True로 강제합니다.

    Args:
        args (Any): 설정 객체. shape: ()
        unnorm_best_traj (torch.Tensor):
            모델이 고른 best 궤적. shape: (B, 1+Pnn, 1+future_len, 4)
        expert_distance_m (torch.Tensor):
            best와 GT의 거리. shape: (B, 1+Pnn)
        unnorm_outputs_copy (Dict[str, torch.Tensor]):
            현재 스텝의 GT/valid 등이 들어있는 dict. shape: ()
        target_future_gt_is_valid (torch.Tensor):
            GT 미래 유효 마스크. shape: (B, 1+Pnn, future_len)
        target_current_is_valid_expand (torch.Tensor):
            현재 포함 유효 마스크. shape: (B, 1+Pnn, 1+future_len)
        future_len (int): 미래 길이. shape: ()

    Returns:
        torch.Tensor:
            저장용 recovery 궤적(현재 포함). shape: (B, 1+Pnn, 1+future_len, 4)
    """
    # args를 직접 바꾸면 execute 로직까지 영향을 줄 수 있어, 얕은 복사로 안전하게 처리합니다.
    shadow_args = copy.copy(args)

    # 저장용은 recovery를 항상 계산하도록 강제
    setattr(shadow_args, "use_recovery", True)
    # 저장용은 threshold를 항상 0m로 가정
    setattr(shadow_args, "recovery_threshold_m", 0.0)

    return _apply_recovery_if_needed(
        args=shadow_args,
        unnorm_best_traj=unnorm_best_traj,
        expert_distance_m=expert_distance_m,
        unnorm_outputs_copy=unnorm_outputs_copy,
        target_future_gt_is_valid=target_future_gt_is_valid,
        target_current_is_valid_expand=target_current_is_valid_expand,
...
def _predict_one_rollout_sequential(
    ...
    with torch.inference_mode():
        ...
        while step_start < int(scenario_finish_step):
            ...
            # ------------------------------------------------------------
            # ✅ (추가) 저장용 GT에 쓸 trajectory를 별도로 준비
            # - execute(실제 이동)는 기존 로직 그대로: unnorm_selected_traj_raw (threshold 기준)
            # - 저장용 GT는 move_by_recovery=True일 때만:
            #     threshold=0m로 가정한 recovery 궤적을 "항상" 계산해서 사용
            # ------------------------------------------------------------
            unnorm_selected_gt_traj_for_save = unnorm_selected_gt_traj  # 기본: 기존 방식 그대로

            if move_by_recovery:
                # (B, 1+Pnn, 1+future_len, 4)  threshold=0m 가정 recovery raw
                unnorm_recovery_traj_raw_for_save = _build_recovery_traj_raw_for_dataset_save(
                    args=args,
                    unnorm_best_traj=unnorm_best_traj,
                    expert_distance_m=best_dist_m,
                    unnorm_outputs_copy=unnorm_outputs_copy,
                    target_future_gt_is_valid=target_future_gt_is_valid,
                    target_current_is_valid_expand=target_current_is_valid_expand,
                    future_len=int(future_len),
                ...
                # GT가 없는 칸은 0으로 마스킹(기존 저장 규칙 유지)
                unnorm_selected_gt_traj_for_save = _apply_unvalid_at_unnorm_selected_traj_raw(
                    unnorm_recovery_traj_raw_for_save,
            ...
            if args.save_inference_data:
                ...
                (ego_future_gt_4_dim, near_future_gt_4_dim,
                 future_seg_control_gt_3_dim
                    ...
                    unnorm_selected_gt_traj=unnorm_selected_gt_traj_for_save,
                    # ✅ 여기만 변경
                    # (B, 1+Pnn, 1+future_len, 4)
            ...
            (unnorm_inputs_copy,
             unnorm_outputs_copy) = _update_merged_inputs_unnorm_inplace_for_time_chunk(
                unnorm_inputs_copy=unnorm_inputs_copy,
                unnorm_outputs_copy=unnorm_outputs_copy,
...
def _transform_origin(
    ...
    *,
    ...
    추가 동작(요청사항)
    - rotate_seg_control_xy=True 이면:
      past/future seg control의 (vx, vy)도 같은 회전으로 변환합니다.
    - state_11[..., 4:6] 변환은 기존 로직을 그대로 사용합니다(여기서 따로 건드리지 않음).

    Args:
        unnorm_inputs_copy: 입력 dict. shape: ()
        unnorm_outputs_copy: 출력/정답 dict. shape: ()
        unnorm_ego_new_cur_pose: (B,4) (x,y,cos,sin)
        cached_valid_masks: lanes/route_lanes/static_objects 마스크 캐시
        rotate_seg_control_xy: seg control (vx,vy) 회전 여부

    Returns:
        Dict[str, torch.Tensor]: 갱신된 입력 dict (참조 그대로 반환)
    """
    ...
    # ✅ 4) seg controls (past/future) (vx, vy) 회전 (필요할 때만)
...
def _should_rotate_seg_control_xy_in_origin_transform(args: Any) -> bool:
    """origin 좌표 변환에서 seg control의 (vx, vy)를 회전할지 결정합니다.

    조건(요청사항)
    - pose_based=False 이고
    - use_body_vel=False 인 경우에만 True
    ...
    Returns:
        bool:
            True면 seg control의 (vx, vy) 회전을 수행합니다. shape: ()
    """
    pose_based = bool(getattr(args, "pose_based", True))
...
def _transform_seg_control_3_dim_xy_inplace(
    ...
    - valid_mask가 있으면 True인 칸만 회전합니다.

    Args:
        control_3 (torch.Tensor):
            control 텐서. shape: (B, ..., 3)
        cos_delta (torch.Tensor):
            새 기준 회전 cos. shape: (B,)
        sin_delta (torch.Tensor):
            새 기준 회전 sin. shape: (B,)
    ...
    if valid_mask is None or (not isinstance(valid_mask, torch.Tensor)) or valid_mask.numel() == 0:
    ...
    # shape이 안 맞으면(예외 케이스) 안전하게 전체 회전 처리
    if got != expected:
        control_3[..., 0:2] = v_xy_new
        return
