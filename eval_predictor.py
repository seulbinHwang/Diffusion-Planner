...
def _as_numpy_array_strict(value: Any, key_name: str) -> np.ndarray:
    """값을 numpy 배열로 바꿉니다(비교/검증용).

    Args:
        value (Any): numpy 배열 / torch 텐서 / 리스트 등. shape: (임의)
        key_name (str): 에러 메시지에 사용할 이름. shape: ()

    Returns:
        np.ndarray: numpy 배열. shape: value와 동일(가능한 경우)
    """
...
def _expand_norm_inputs_for_rollout_batch(
    ...
    for k, v in norm_inputs.items():
        norm_inputs_b_r_copy[k] = _expand_value_for_rollout_batch(
            value=v,
            batch_size=int(batch_size),
            rollout_repeat=int(rollout_repeat),
        )
    norm_outputs_b_r_copy: Dict[str, Any] = {}
    for k, v in norm_outputs.items():
        if k not in norm_outputs_b_r_copy:
            norm_outputs_b_r_copy[k] = _expand_value_for_rollout_batch(
                value=v,
                batch_size=int(batch_size),
                rollout_repeat=int(rollout_repeat),
            )
    return norm_inputs_b_r_copy, norm_outputs_b_r_copy
...
def _build_draw_current_valid_pad(
    ...
    if isinstance(cur_valid, torch.Tensor):
        if cur_valid.dim() == 2 and int(cur_valid.shape[1]) == a and int(
                cur_valid.shape[0]) > int(draw_batch_idx):
    ...
    near_past_valid = norm_inputs_b_r_copy.get("near_agents_past_is_valid",
                                               None)
    if isinstance(ego_past_valid, torch.Tensor) and isinstance(
            near_past_valid, torch.Tensor):
        if ego_past_valid.dim() == 2 and near_past_valid.dim() == 3:
            if int(ego_past_valid.shape[0]) > int(draw_batch_idx) and int(
                    near_past_valid.shape[0]) > int(draw_batch_idx):
                ego_cur = ego_past_valid[int(draw_batch_idx),
                                         -1].reshape(1)  # (1,)
                near_cur = near_past_valid[int(draw_batch_idx), :, -1]  # (Pnn,)
                if int(near_cur.shape[0]) + 1 == a:
                    row = torch.cat([ego_cur, near_cur],
                                    dim=0).to(device=device)  # ((1+)Pnn,)
    ...
    if isinstance(tfv, torch.Tensor):
        if tfv.dim() == 3 and int(tfv.shape[1]) == a and int(
                tfv.shape[0]) > int(draw_batch_idx) and int(tfv.shape[2]) >= 1:
...
def _get_unnorm_target_pose_chunk(
    ...
    하는 일
    ------
    - normed_trajectories에서 ego/near의 1..gap 구간 포즈를 꺼냅니다.
    - ego와 near를 합쳐서 (B*R, (1+)Pnn, gap, 4) 모양으로 만듭니다.
    - target_future_valid의 앞 gap 구간을 valid_mask로 사용해서,
      state_normalizer.inverse(...)로 원래 단위로 되돌립니다.
      (valid_mask가 False인 칸은 0으로 남게 됩니다)


    Returns:
        torch.Tensor:
            원래 단위로 되돌린 target 포즈 chunk.
            shape: (B*R, (1+)Pnn, gap, 4)
            마지막 4는 (x, y, cos, sin) 입니다.
    """
    if int(gap) <= 0:
    ...
    if target_future_control_seq is None:
        ...
        unnorm_target_future_control_seq = state_normalizer.inverse(
            data=target_future_control_seq, valid_mask=target_future_valid)
        # (B*R, (1+)Pnn, T, 3) ->  (B*R, (1+)Pnn, gap, 3)
        unnorm_target_control_chunk = unnorm_target_future_control_seq[:, :, :
                                                                       gap, :]  #
    ...
    # target_future_valid: (B*R, (1+)Pnn, future_len)
    # target_gap_valid: (B*R, (1+)Pnn, gap)
    target_gap_valid = target_future_valid[:, :, :gap]
...
def _convert_target_chunk_to_world(
        ...
        target_chunk_valid: Optional[
            torch.Tensor] = None,  # (B*R, (1+)Pnn, gap)
    ...
    if target_chunk_valid is not None:
        ...
        if int(target_chunk_valid.shape[0]) != merged_batch or int(
                target_chunk_valid.shape[1]) != one_or_pnn or int(
                    target_chunk_valid.shape[2]) != int(gap):
            raise ValueError(
        ...
        target_chunk_valid_flat = target_chunk_valid.reshape(
            -1, int(gap))  # (N, gap)
    ...
    # (B*R, (1+)Pnn, gap, 4)
    return unnorm_target_pose_chunk_world_flat.reshape(merged_batch, one_or_pnn,
                                                       int(gap), 4)
...
def _update_target_seg_control_for_time_chunk(
    ...
    past_seg_control_gt_3_dim = unnorm_inputs_b_r_copy.get(
    ...
    if past_seg_control_gt_3_dim.dim() != 4 or int(
            past_seg_control_gt_3_dim.shape[-1]) != 3:
        raise ValueError(
            "past_seg_control_gt_3_dim은 (B*R, 1+Pnn, past_len, 3) 이어야 합니다. "
            f"got shape={tuple(past_seg_control_gt_3_dim.shape)}")

    if unnorm_target_control_chunk.dim() != 4 or int(
            unnorm_target_control_chunk.shape[-1]) != 3:
        raise ValueError(
            "unnorm_target_control_chunk는 (B*R, 1+Pnn, gap, 3) 이어야 합니다. "
            f"got shape={tuple(unnorm_target_control_chunk.shape)}")

    if int(past_seg_control_gt_3_dim.shape[0]) != int(
            unnorm_target_control_chunk.shape[0]) or int(
                past_seg_control_gt_3_dim.shape[1]) != int(
    ...
    """
    ego_agent_cur_is_valid = unnorm_inputs_b_r_copy[
        "ego_agent_past_is_valid"][:, -1:]  # (B*R, 1)
    near_agents_cur_is_valid = unnorm_inputs_b_r_copy[
        "near_agents_past_is_valid"][:, :, -1]  # (B, Pnn,)
    current_seg_control_is_valid = torch.cat(
        [ego_agent_cur_is_valid, near_agents_cur_is_valid],
        dim=1)  # (B*R, 1+Pnn)
    current_seg_control_is_valid = current_seg_control_is_valid.unsqueeze(
        -1)  # (B*R, 1+Pnn, 1)
    # current_seg_chunk_control_is_valid :  (B*R, 1+Pnn, 1) -> expand -> (B*R, 1+Pnn, gap)
    current_seg_chunk_control_is_valid = current_seg_control_is_valid.expand(
        -1, -1, gap)
    ...
    masked_control = _match_device_and_dtype(masked_control,
    ...
    left = past_seg_control_gt_3_dim[:, :,
                                     gap:, :]  # (B*R, 1+Pnn, past_len-move, 3)
    unnorm_inputs_b_r_copy["past_seg_control_gt_3_dim"] = torch.cat(
        [left, masked_control], dim=2)

    # past_seg_control_is_valid: (B*R, 1+Pnn, past_len) 이것도 업데이트 해야함
    past_seg_control_is_valid = unnorm_inputs_b_r_copy[
        "past_seg_control_is_valid"]  # (B*R, 1+Pnn, past_len)
    unnorm_inputs_b_r_copy["past_seg_control_is_valid"] = torch.cat([
        past_seg_control_is_valid[:, :, gap:],
        current_seg_chunk_control_is_valid
    ],
                                                                    dim=2)
...
def _predict_rollouts_batched_one_chunk(
    ...
    target_joint_scene_world = torch.empty(
        ...
        device=ego_future_gt_4_dim.device,
        dtype=ego_future_gt_4_dim.dtype,
    )

    # ✅ time-chunk 크기 N
    requested_time_chunk_size = int(
        max(1, int(getattr(args, "rollout_time_chunk_size", 1))))

    ego_agent_past_unnorm = unnorm_inputs_b_r_copy["ego_agent_past"]
    time_len = int(max(1, int(ego_agent_past_unnorm.shape[1])))
    ...
    with torch.inference_mode():
        ...
        while step_start < future_len:
            ...
            # ---------------------------------------------------------
            # ✅ (핵심 수정) save_image=True일 때만 draw 준비/CPU/NumPy 변환 수행
            # ---------------------------------------------------------
            if save_image:
                ...
                (unnorm_inputs_np, unnorm_trajectory_np, near_future_gt_3_dim,
                 ego_future_gt_4_dim_np) = _prepare_data_for_one_batch_draw(
                     ...
                     norm_outputs_b_r_copy=norm_outputs_b_r_copy,
                     normed_trajectories=
                     normed_trajectories,  # 이번 step에서 출력한 diffusion 궤적
                     state_normalizer=state_normalizer,
                     observation_normalizer=observation_normalizer,
                ...
                _draw_one_batch_one_rollout(
                    ...
                    unnorm_trajectory_np,  # 이번 step에서 출력한 diffusion 궤적
            ...
            (unnorm_target_pose_chunk, unnorm_target_control_chunk
            ) = _get_unnorm_target_pose_chunk(
            ...
            target_chunk_valid = norm_inputs_b_r_copy[
                "target_future_valid"][:, :, :gap]
...
def _to_debug_object_id_str(object_id: Any) -> str:
    """디버그 dict key로 쓸 object_id를 사람이 보기 쉬운 문자열로 만듭니다.

    목적
    ----
    torch.Tensor(스칼라)을 f-string으로 바로 문자열로 만들면
    "tensor(123)" 같은 형태가 되어, 로그/디버그 해석이 불편해질 수 있습니다.
    그래서 스칼라 텐서는 숫자(int)로 꺼내서 "123" 형태로 만듭니다.

    Args:
        object_id (Any):
            보통 다음 중 하나를 기대합니다.
            - torch.Tensor 스칼라: shape=() 또는 (1,)
            - numpy 스칼라: np.int64 등
            - int / str 등
    ...
    if isinstance(object_id, torch.Tensor):
        if int(object_id.numel()) != 1:
            raise ValueError(
                "object_id 텐서는 스칼라(원소 1개)여야 합니다. "
                f"got numel={int(object_id.numel())}, shape={tuple(object_id.shape)}"
            )
        return str(int(object_id.detach().cpu().item()))

    if isinstance(object_id, np.generic):
        return str(int(object_id.item()))
...
def _draw_one_batch_one_rollout(
    ...
    for target_id, gt_future_3 in zip(draw_near_target_id,
        ...
        key = _to_debug_object_id_str(target_id)
    ...
    for target_id, np_int_traj_11 in zip(draw_near_target_id, near_future_11):
        key = _to_debug_object_id_str(target_id)
        diff_token_to_np_int_traj_11_wrt_ego[key] = np_int_traj_11  # (1+T, 11)
...
def _slice_amortized_eps_from_bank(
    ...
    *,
    ...
    """
    if eps_bank.dim() != 4:
        raise ValueError("eps_bank는 (B*R, A, bank_len, D) 형태여야 합니다. "
...
def _slice_inference_noise_from_bank(
    ...
    """
    if noise_bank.dim() != 4:
        raise ValueError("noise_bank는 (B*R, A, bank_len, D) 형태여야 합니다. "
...
def _sanitize_inputs_for_validation(inputs: Dict[str, Any],) -> Dict[str, Any]:
...
def _expand_target_z_to_rollout_grid(
    ...
    --------------
    - 입력: target_z: (B, (1+)Pnn)
    - 출력: target_z_grid: (B*(1+)Pnn, R, future_len)
      - N = B*(1+)Pnn
      - future_len = n_step
...
def validate_func(
    ...
    assert args.eval_method == "validation", (
        "validate_func는 args.eval_method가 'validation'일 때만 호출되어야 합니다.")
...
* **_predict_rollouts_batched_one_chunk = “한 묶음의 rollout을 실제로 생성”**
    ...
    # wosac_agent_is_valid: (N=B*(1+)Pnn)
    wosac_agent_is_valid = target_agents_current_valid.reshape(-1).to(
        dtype=torch.bool)  # (N,)
    ...
    (
        ...
        pred_z=pred_z,  # (N, R, future_len)
        pred_head=pred_head,  # (N, R, future_len)
        agent_valid_mask=wosac_agent_is_valid,  # (N,)
    )
    # ------------------------------------------------------------
...
def _initialize_unnorm_inputs_for_rollout(
    ...
    unnorm_outputs_b_r_copy["near_future_gt_4_dim"] = state_normalizer.inverse(
        ...
        valid_mask=norm_outputs_b_r_copy["near_future_gt_is_valid"])
    unnorm_outputs_b_r_copy[
        "future_seg_control_gt_3_dim"] = state_normalizer.inverse(
...
def _enforce_unit_cos_sin_in_state_11_inplace(
    ...
    if state_11.dim() < 1 or int(state_11.shape[-1]) != 11:
        raise ValueError("state_11은 (..., 11) 형태여야 합니다. "
...
def _build_cached_valid_masks_for_static_map_features(
    unnorm_inputs_b_r_copy: Dict[str, Any],) -> Dict[str, torch.Tensor]:
    ...
    lanes_len_is_valid = unnorm_inputs_b_r_copy.get("lanes_len_is_valid", None)
    if isinstance(lanes_len_is_valid,
                  torch.Tensor) and lanes_len_is_valid.numel() > 0:
        # lanes: (B*R, lane_num, lane_len)
        cached["lanes"] = lanes_len_is_valid.to(dtype=torch.bool)

    route_lanes_len_is_valid = unnorm_inputs_b_r_copy.get(
    ...
    static_objects_is_valid = unnorm_inputs_b_r_copy.get(
...
def _pick_cached_mask_if_shape_matches(
    ...
    Returns:
        Optional[torch.Tensor]:
            ...
            - 없거나 shape가 다르면 None
    """
    if cached_valid_masks_br is None:
        return None

    mask = cached_valid_masks_br.get(key, None)
...
def _update_ego_past_and_valid_inplace_for_time_chunk(
    ...
    Args:
        unnorm_inputs_b_r_copy (Dict[str, Any]):
            ...
            필요한 키/shape:
              - ego_agent_past: (B*R, time_len, 11)
              - ego_agent_past_is_valid: (B*R, time_len)
        unnorm_ego_pose_chunk (torch.Tensor):
            이번에 예측된 ego 포즈 시퀀스.
    ...
    """
    ego_agent_past = unnorm_inputs_b_r_copy[
        "ego_agent_past"]  # (B*R, time_len, 11)
...
def _assert_non_near_agents_not_supported_for_time_chunk(
    unnorm_inputs_b_r_copy: Dict[str, Any],) -> None:
    ...
    """
    non_near_agents_past = unnorm_inputs_b_r_copy.get("non_near_agents_past",
                                                      None)
...
def _update_neighbor_past_and_valid_inplace_for_time_chunk(
    ...
    Args:
        unnorm_inputs_b_r_copy (Dict[str, Any]):
            입력 dict(원래 단위, inplace 갱신).
            사용하는 키/shape:
              - neighbor_agents_past: (B*R, Pnn, time_len, 11) 또는 None
              - near_agents_past: (B*R, Pnn, time_len, 11)
              - near_agents_past_is_valid: (B*R, Pnn, time_len)
            생성/갱신하는 키:
              - neighbor_agents_past
              - neighbor_agents_past_is_valid
              - near_agents_is_valid: (B*R, Pnn)  # 마지막 time 스텝의 valid
              - neighbor_agents_is_valid: (B*R, Pnn)


    Returns:
        None
    """
    neighbor_agents_past = unnorm_inputs_b_r_copy.get("neighbor_agents_past",
                                                      None)
    if neighbor_agents_past is None:
        unnorm_inputs_b_r_copy["neighbor_agents_past"] = None
        return

    near_agents_past = unnorm_inputs_b_r_copy["near_agents_past"]
    near_agents_past_is_valid = unnorm_inputs_b_r_copy[
        "near_agents_past_is_valid"]

    # neighbor_agents_past: near를 그대로 복사
    unnorm_inputs_b_r_copy["neighbor_agents_past"] = near_agents_past.clone()
    ...
    unnorm_inputs_b_r_copy["near_agents_is_valid"] = neighbor_agents_is_valid
    unnorm_inputs_b_r_copy[
        "neighbor_agents_is_valid"] = neighbor_agents_is_valid
...
def _update_future_gt_and_valid_inplace_for_time_chunk(
    ...
    Args:
        unnorm_inputs_b_r_copy (Dict[str, Any]):
            입력 dict(원래 단위, inplace 갱신).
            갱신하는 키:
              - ego_future_gt_is_valid: (B*R, future_len) bool
              - near_future_gt_is_valid: (B*R, Pnn, future_len) bool
              - neighbor_future_gt_is_valid: (B*R, Pnn, future_len) bool
    ...
    ego_future_gt_4_dim = unnorm_outputs_b_r_copy[
        "ego_future_gt_4_dim"]  # (B*R, future_len, 4)
    ...
    # near future shift
    near_future_gt_4_dim = unnorm_outputs_b_r_copy[
        "near_future_gt_4_dim"]  # (B*R, Pnn, future_len, 4)
    near_future_gt_4_dim[:, :, :future_len -
                         gap, :] = near_future_gt_4_dim[:, :, gap:, :].clone()
    near_future_gt_4_dim[:, :, future_len - gap:, :] = 0.0
    unnorm_outputs_b_r_copy["near_future_gt_4_dim"] = near_future_gt_4_dim

    near_future_gt_is_valid = unnorm_outputs_b_r_copy[
    ...
    """
    future_seg_control_gt_3_dim = unnorm_outputs_b_r_copy[
        "future_seg_control_gt_3_dim"]
    future_seg_control_gt_3_dim[:, :, :future_len -
                                gap, :] = future_seg_control_gt_3_dim[:, :,
                                                                      gap:, :].clone(
                                                                      )
    ...
    future_seg_control_is_valid[:, :, :future_len -
                                gap] = future_seg_control_is_valid[:, :,
                                                                   gap:].clone(
                                                                   )
    future_seg_control_is_valid[:, :, future_len - gap:] = 0
    future_seg_control_is_valid = future_seg_control_is_valid.to(
        dtype=torch.bool)
    unnorm_outputs_b_r_copy[
        "future_seg_control_is_valid"] = future_seg_control_is_valid
...
def _transform_origin_step1_ego_past_inplace(
    ...
    Returns:
        None
    """
    ego_agent_past = unnorm_inputs_b_r_copy["ego_agent_past"]
    ego_agent_past_is_valid = unnorm_inputs_b_r_copy["ego_agent_past_is_valid"]
    _transform_state_11_dim_inplace(
        state_11=ego_agent_past,  # (B*R, time_len, 11)
        delta_xy=delta_xy,  # (B*R, 2)
        cos_delta=cos_delta,  # (B*R,)
        sin_delta=sin_delta,  # (B*R,)
        valid_mask=ego_agent_past_is_valid,  # (B*R, time_len)
    )
...
def _transform_origin_step2_agents_past_inplace(
    ...
    Args:
        unnorm_inputs_b_r_copy: 입력 dict (inplace 변경).
            - near_agents_past: (B*R, Pnn, time_len, 11)
            - non_near_agents_past: (B*R, Nnn, time_len, 11) 또는 (B*R, 0, time_len, 11)
            - neighbor_agents_past: (B*R, Pnn, time_len, 11) 또는 None
            - 각 *_is_valid: (B*R, P?, time_len) (없을 수도 있음)
        delta_xy: 새 기준의 위치 이동 값. shape (B*R, 2)
        cos_delta: 새 기준의 방향 cos 값. shape (B*R,)
        sin_delta: 새 기준의 방향 sin 값. shape (B*R,)
    ...
    for key, valid_key in (
        ...
        agents_past = unnorm_inputs_b_r_copy.get(key, None)
        if not isinstance(agents_past,
                          torch.Tensor) or agents_past.numel() == 0:
            continue

        valid_mask = unnorm_inputs_b_r_copy[valid_key]

        _transform_state_11_dim_inplace(
            state_11=agents_past,  # (B*R, P?, time_len, 11)
            delta_xy=delta_xy,  # (B*R, 2)
            cos_delta=cos_delta,  # (B*R,)
            sin_delta=sin_delta,  # (B*R,)
            valid_mask=valid_mask,  # (B*R, P?, time_len)
        )
...
def _transform_origin_step3_future_gt_inplace(
    ...
    Args:
        unnorm_outputs_b_r_copy: 출력/정답 dict (inplace 변경).
            - ego_future_gt_4_dim: (B*R, future_len, 4)
            - near_future_gt_4_dim: (B*R, Pnn, future_len, 4)
            - 각 *_is_valid: (B*R, ...) (없을 수도 있음)
        delta_xy: 새 기준의 위치 이동 값. shape (B*R, 2)
        cos_delta: 새 기준의 방향 cos 값. shape (B*R,)
        sin_delta: 새 기준의 방향 sin 값. shape (B*R,)
    ...
    for pose_key, valid_key in (
        ...
        pose_4 = unnorm_outputs_b_r_copy.get(pose_key, None)
        if not isinstance(pose_4, torch.Tensor) or pose_4.numel() == 0:
            continue

        valid_mask = unnorm_outputs_b_r_copy[valid_key]
...
def _transform_origin_step5_points_inplace(
    ...
    for point_key, valid_key in point_and_mask_keys:
        points_xy = unnorm_inputs_b_r_copy.get(point_key, None)

        if not isinstance(points_xy, torch.Tensor) or points_xy.numel() == 0:
            continue
        points_valid = unnorm_inputs_b_r_copy[valid_key]

        # points_valid: (B*R, N) + points_xy: (B*R, N, L, 2) 인 케이스 대응
        if points_valid.dim() == 2 and points_xy.dim() == 4:
            points_valid = points_valid.unsqueeze(-1).expand_as(
                points_xy[..., 0])  # (B*R, N, L)

        _transform_points_2d_inplace(
            points_xy=points_xy,  # (B*R, ..., 2)
            delta_xy=delta_xy,  # (B*R, 2)
            cos_delta=cos_delta,  # (B*R,)
            sin_delta=sin_delta,  # (B*R,)
            valid_mask=points_valid.to(dtype=torch.bool),  # (B*R, ...)
        )
...
def _transform_origin_step6_lanes_inplace(
    ...
    Args:
        ...
        cos_delta: 새 기준의 방향 cos 값. shape (B*R,)
        sin_delta: 새 기준의 방향 sin 값. shape (B*R,)
        cached_valid_masks_br: 미리 만든 유효 마스크 dict.
            - "lanes": (B*R, lane_num, lane_len) bool
            - "route_lanes": (B*R, route_lane_num, lane_len) bool
    ...
    for lane_key in ("lanes", "route_lanes"):
        lane_12 = unnorm_inputs_b_r_copy.get(lane_key, None)
        ...
        cached_mask = _pick_cached_mask_if_shape_matches(
            cached_valid_masks_br=cached_valid_masks_br,
            key=lane_key,
            expected_shape=lane_12.shape[:-1],  # (B*R, lane_num, lane_len)
        )
        ...
        _transform_lane_12_dim_inplace(
            lane_12=lane_12,  # (B*R, ..., 12)
            delta_xy=delta_xy,  # (B*R, 2)
            cos_delta=cos_delta,  # (B*R,)
            sin_delta=sin_delta,  # (B*R,)
            valid_mask=cached_mask,  # (B*R, ...)
        )
...
def _transform_origin_step7_static_objects_inplace(
    ...
    Returns:
        None
    """
    static_objects = unnorm_inputs_b_r_copy.get("static_objects", None)
    ...
    cached_mask = _pick_cached_mask_if_shape_matches(
        cached_valid_masks_br=cached_valid_masks_br,
        key="static_objects",
        expected_shape=static_objects.shape[:-1],  # (B*R, static_num)
    )
    if cached_mask is None:
        raise NotImplementedError(
    ...
    _transform_static_object_10_dim_inplace(
        static_10=static_objects,  # (B*R, static_num, 10)
        delta_xy=delta_xy,  # (B*R, 2)
        cos_delta=cos_delta,  # (B*R,)
        sin_delta=sin_delta,  # (B*R,)
        valid_mask=cached_mask,  # (B*R, static_num)
    )
...
def _extract_delta_pose_params(
    ...
    Args:
        unnorm_ego_new_cur_pose: (B, 4) = (x, y, cos(각도), sin(각도))
    ...
    Returns:
        ...
        yaw_delta: (B,)
    """
    if unnorm_ego_new_cur_pose.dim(
    ) != 2 or unnorm_ego_new_cur_pose.shape[-1] != 4:
        raise ValueError("normed_ego_next_pose는 (B, 4) 여야 합니다. "
                         f"현재 shape={tuple(unnorm_ego_new_cur_pose.shape)}")

    delta_xy = unnorm_ego_new_cur_pose[:, 0:2]  # (B, 2)
    cos_raw = unnorm_ego_new_cur_pose[:, 2]  # (B,)
    sin_raw = unnorm_ego_new_cur_pose[:, 3]  # (B,)
...
def _transform_seg_control_3_dim_xy_inplace(
    ...
    if control_3.dim() < 2 or int(control_3.shape[-1]) != 3:
        raise ValueError("control_3는 (..., 3)이어야 합니다. "
...
def _transform_state_11_dim_inplace(
    ...
    추가 동작
    --------
    - valid_mask가 False인 위치는 state_11[..., :8] 값을 0으로 채웁니다.
    """
    ...
    pos_xy_new = _transform_points_to_new_origin(pos_xy, delta_xy, cos_delta,
                                                 sin_delta)
    state_11[..., 0:2] = torch.where(valid_mask_bool[..., None], pos_xy_new,
                                     pos_xy)
    ...
    state_11[..., 2] = torch.where(valid_mask_bool, cos_new, cos_h)
    state_11[..., 3] = torch.where(valid_mask_bool, sin_new, sin_h)

    vel_xy = state_11[..., 4:6]
    vel_xy_new = _rotate_vectors_to_new_origin(vel_xy, cos_delta, sin_delta)
    state_11[..., 4:6] = torch.where(valid_mask_bool[..., None], vel_xy_new,
                                     vel_xy)

    # ✅ 추가: valid_mask가 False인 곳은 앞 8차원을 0으로 채움
    state_11[..., :8] = torch.where(
        valid_mask_bool[..., None],
        state_11[..., :8],
        torch.zeros_like(state_11[..., :8]),
    )
...
def _covert_from_ego_to_world(
    ...
    if valid_mask is None:
        ...
        if got_shape != expected_shape:
            raise ValueError("valid_mask shape가 target_poses와 맞지 않습니다. "
        ...
        valid_mask_bool = valid_mask_bool.to(device=world_pose.device,
                                             dtype=torch.bool)

    world_pose = torch.where(valid_mask_bool[..., None], world_pose,
...
def _enforce_pose_based_requires_feasible(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "pose_based=False인데 args.use_feasible을 True로 강제 설정하지 못했습니다.") from e

    # (선택) 사용자 실수 방지용 로그: 분산 초기화 전이라 env RANK로 판단
    if (not prev_use_feasible) and int(_get_rank_from_env()) == 0:
        print("[CONFIG] pose_based=False 이므로 use_feasible=True 로 강제 설정합니다.",
              flush=True)

    assert bool(
        getattr(args, "use_feasible",
                False)), "pose_based=False일 때는 use_feasible=True가 반드시 필요합니다."
