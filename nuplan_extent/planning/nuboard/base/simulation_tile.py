...
from nuplan_extent.planning.nuboard.base.plot_data import MapPoint, SimulationData, SimulationFigure
...
class SimulationTile:
    ...
    def __init__(
        ...
        self._doc.add_periodic_callback(self._periodic_callback,
        ...
        logger.info("Minimum frame time=%4.3f s",
                    self._minimum_frame_time_seconds)
    ...
    def _create_frame_control_button(
            self, button_config: ScenarioTabFrameButtonConfig,
            click_callback: EventCallback, figure_index: int) -> Button:
        """
        ...
        button_instance.on_click(
            partial(click_callback, figure_index=figure_index))
    ...
    def _create_initial_figure(
        ...
        experiment_path = Path(self._experiment_file_data.file_paths[
            selected_scenario_key.nuboard_file_index].metric_main_path)
        ...
        simulation_figure.on_event(
            "mousemove", partial(self._on_mouse_move,
        ...
        slider.on_change(
            "value", partial(self._slider_on_change, figure_index=figure_index))
        video_button = Button(
            label="Render video",
        ...
        video_button.on_click(
            partial(self._video_button_on_click, figure_index=figure_index))

        first_button = self._create_frame_control_button(
            first_button_config, self._first_button_on_click, figure_index)
        prev_button = self._create_frame_control_button(
            prev_button_config, self._prev_button_on_click, figure_index)
        play_button = self._create_frame_control_button(
            play_button_config, self._play_button_on_click, figure_index)
        next_button = self._create_frame_control_button(
            next_button_config, self._next_button_on_click, figure_index)
        last_button = self._create_frame_control_button(
            last_button_config, self._last_button_on_click, figure_index)

        assert len(
            selected_scenario_key.files
        ) == 1, "Expected one file containing the serialized SimulationLog."
    ...
    def init_simulations(self, figure_sizes: List[int]) -> None:
        ...
        for figure_index in range(len(self._selected_scenario_keys)):
            simulation_figure = self._create_initial_figure(
                figure_index=figure_index, figure_sizes=figure_sizes)
    ...
    def _video_button_on_click(self, figure_index: int) -> None:
        ...
        self._figures[
            figure_index].video_button.label = "Rendering video now..."
    ...
    def _video_button_next_tick(self, figure_index: int) -> None:
        ...
        video_path = (
            Path(self._experiment_file_data.file_paths[nuboard_file_index].
                 simulation_main_path) / "video_screenshot")
        ...
        try:
            if len(selected_simulation_figure.ego_state_plot.data_sources):
                ...
                video_obj = cv2.VideoWriter(filename=str(video_save_path),
                                            fourcc=fourcc,
                                            fps=fps,
                                            frameSize=shape)
        ...
        self._doc.add_next_tick_callback(
            partial(self._reset_video_button, figure_index=figure_index))
    ...
    def _last_button_on_click(self, figure_index: int) -> None:
        ...
        """
        figure = self._figures[figure_index]
        self._request_specific_frame(
            figure=figure, frame_index=len(figure.simulation_history.data) - 1)

    def _slider_on_change(self, attr: str, old: int, frame_index: int,
        ...
        self._request_plot_rendering(figure=selected_figure,
                                     frame_index=frame_index)

    def _request_specific_frame(self, figure: SimulationFigure,
    ...
    def _request_plot_rendering(self, figure: SimulationFigure,
                                frame_index: int) -> None:
        """
        ...
        if current_time - self._last_frame_time < self._minimum_frame_time_seconds or figure.is_rendering(
            ...
            self._process_plot_render_request(figure=figure,
                                              frame_index=frame_index)
            self._last_frame_time = time.time()
    ...
    def _start_playback(self, figure: SimulationFigure) -> None:
        ...
        callback_period_seconds = max(self._minimum_frame_time_seconds,
                                      callback_period_seconds)
    ...
    def _process_plot_render_request(self, figure: SimulationFigure,
                                     frame_index: int) -> None:
        """
        ...
        if self._async_rendering:
            ...
            def render() -> None:
                ...
                if mission_goal is not None:
                    main_figure.render_mission_goal(
                        mission_goal_state=mission_goal)

                self._render_plots(main_figure=main_figure,
                                   frame_index=0,
                                   hidden_glyph_names=hidden_glyph_names)
    ...
    def _load_map_data(self, main_figure: SimulationFigure) -> None:
        ...
        self._nearest_vector_map = map_api.get_proximal_map_objects(
            center, self._radius, layer_names)
        ...
        if SemanticMapLayer.STOP_LINE in self._nearest_vector_map:
            ...
            self._nearest_vector_map[SemanticMapLayer.STOP_LINE] = [
                stop_polygon for stop_polygon in stop_polygons
    ...
    def _render_map_polygon_layers(self, main_figure: SimulationFigure) -> None:
        ...
        polygon_layer_names = [
            (SemanticMapLayer.LANE,
             simulation_map_layer_color[SemanticMapLayer.LANE]),
            (SemanticMapLayer.INTERSECTION,
             simulation_map_layer_color[SemanticMapLayer.INTERSECTION]),
            (SemanticMapLayer.STOP_LINE,
             simulation_map_layer_color[SemanticMapLayer.STOP_LINE]),
            (SemanticMapLayer.CROSSWALK,
             simulation_map_layer_color[SemanticMapLayer.CROSSWALK]),
            (SemanticMapLayer.WALKWAYS,
             simulation_map_layer_color[SemanticMapLayer.WALKWAYS]),
            (SemanticMapLayer.CARPARK_AREA,
        ...
        if roadblock_ids:
            polygon_layer_names.append(
                (SemanticMapLayer.ROADBLOCK,
                 simulation_map_layer_color[SemanticMapLayer.ROADBLOCK]))
        ...
        for layer_name, color in polygon_layer_names:
            ...
            main_figure.map_polygon_plots[
                layer_name.name] = layer_map_polygon_plot
    ...
    def _render_map_line_layers(self, main_figure: SimulationFigure) -> None:
        ...
        for layer_name, color in line_layer_names:
            ...
            line_source = ColumnDataSource(
                dict(xs=map_line.line_xs, ys=map_line.line_ys))
    ...
    def _render_plots(self,
        ...
        if main_figure.lane_connectors is not None and len(
                main_figure.lane_connectors):
        ...
        def update_decorations() -> None:
            main_figure.figure.title.text = main_figure.figure_title_name_with_timestamp(
                frame_index=frame_index)
    ...
    def _periodic_callback(self) -> None:
        ...
        if self._plot_render_queue:
            ...
            last_frame_direction = math.copysign(
                1, self._current_frame_index - self._last_frame_index)
            request_frame_direction = math.copysign(
                1, frame_index - self._current_frame_index)
