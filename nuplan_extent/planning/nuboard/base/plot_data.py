...
class BokehAgentStates(NamedTuple):
    ...
    xs: List[List[List[
        List[float]]]]  # [m], [[list of [[Polygon connected corners in x]]]]
    ys: List[List[List[
        List[float]]]]  # [m], [[list of [[Polygon connected corners in y]]]]
...
class MapPoint:
    ...
    point_2d: List[List[Point2D]] = field(
        default_factory=list)  # A list of a list of 2D points
...
class TrafficLightMapLine(MapPoint):
    ...
    line_colors: List[str] = field(
        default_factory=list)  # A list of color hex codes.
    line_color_alphas: List[float] = field(
        default_factory=list)  # A list of color alphas.
...
class BaseScenarioPlot(abc.ABC):
    ...
    data_source_condition: Optional[threading.Condition] = field(default=None,
                                                                 init=False)
...
class TrafficLightPlot(BaseScenarioPlot):
    ...
    data_sources: Dict[int, ColumnDataSource] = field(
        default_factory=dict)  # A dict of data sources for each frame
    plot: Optional[GlyphRenderer] = None  # A bokeh glyph element

    def update_plot(self, main_figure: Figure, frame_index: int,
                    doc: Document) -> None:
        """
        Update the plot.
    ...
    def update_data_sources(self, scenario: AbstractScenario,
                            history: SimulationHistory,
                            lane_connectors: Dict[str, LaneConnector]) -> None:
        """
        ...
        with self.data_source_condition:
            for frame_index in range(len(history.data)):
                traffic_light_status = history.data[
                    frame_index].traffic_light_status

                traffic_light_map_line = TrafficLightMapLine(
                    point_2d=[], line_colors=[], line_color_alphas=[])
                lane_connector_colors = simulation_map_layer_color[
                    SemanticMapLayer.LANE_CONNECTOR]
                for traffic_light in traffic_light_status:
                    lane_connector = lane_connectors.get(
                        str(traffic_light.lane_connector_id), None)
                    ...
                    if lane_connector is not None:
                        ...
                        traffic_light_map_line.line_colors.append(
                            traffic_light.status.name)
                        traffic_light_map_line.line_color_alphas.append(
                            lane_connector_colors["line_color_alpha"])
                ...
                line_source = ColumnDataSource(
                    dict(
                        ...
                        line_color_alphas=traffic_light_map_line.
                        line_color_alphas,
                    ))
...
class EgoStatePlot(BaseScenarioPlot):
    ...
    def update_plot(self, main_figure: Figure, radius: float, frame_index: int,
                    doc: Document) -> None:
        """
        Update the plot.
        :param main_figure: The plotting figure.
            ...
            def update_main_figure() -> None:
                ...
                if self.plot is None:
                    self.plot = main_figure.multi_polygons(
                        xs="xs",
                        ys="ys",
                        fill_color=simulation_tile_agent_style["ego"]
                        ["fill_color"],
                        fill_alpha=simulation_tile_agent_style["ego"]
                        ["fill_alpha"],
                        line_color=simulation_tile_agent_style["ego"]
                        ["line_color"],
                        line_width=simulation_tile_agent_style["ego"]
                        ["line_width"],
                        source=data_sources,
                    )
                    ego_hover = HoverTool(
            ...
            for frame_index, sample in enumerate(history.data):
                ...
                source = ColumnDataSource(
                    dict(
                        ...
                        yaw_rate=[
                            sample.ego_state.dynamic_car_state.angular_velocity
                        ],
                        xs=[[[corner_xs]]],
                        ys=[[[corner_ys]]],
                    ))
...
class EgoStateTrajectoryPlot(BaseScenarioPlot):
            ...
            def update_main_figure() -> None:
                ...
                if self.plot is None:
                    self.plot = main_figure.line(
                        x="xs",
                        y="ys",
                        line_color=simulation_tile_trajectory_style["ego"]
                        ["line_color"],
                        line_width=simulation_tile_trajectory_style["ego"]
                        ["line_width"],
                        line_alpha=simulation_tile_trajectory_style["ego"]
                        ["line_alpha"],
                        source=data_sources,
                    )
                else:
    ...
    def update_data_sources(self, history: SimulationHistory) -> None:
        ...
        with self.data_source_condition:
            for frame_index, sample in enumerate(history.data):
                # # sample : SimulationHistorySample
                # sample.trajectory: InterpolatedTrajectory(AbstractTrajectory)
                # trajectory: List[InterpolatableState]
...
class AgentStatePlot(BaseScenarioPlot):
    ...
    data_sources: Dict[int, Dict[str, ColumnDataSource]] = field(
    ...
    def update_plot(self, main_figure: Figure, frame_index: int,
        ...
        with self.data_source_condition:
            ...
            def update_main_figure() -> None:
                """Wrapper for the main_figure update logic to support multi-threading."""
                # Dict[int, Dict[str, ColumnDataSource]]
    ...
    def update_data_sources(self, history: SimulationHistory) -> None:
        ...
        with self.data_source_condition:
            # history.data :  List[SimulationHistorySample]
            # self.data_sources: Dict[int, Dict[str, ColumnDataSource]]
            for frame_index, sample in enumerate(history.data):
                # sample : SimulationHistorySample
...
tracked_object_types = {
    ...
    'czone_sign': TrackedObjectType.CZONE_SIGN,
                ...
                """
                # 한 시점 내에서, 종류별로 분류
                # 한 시점 내에서, frame_dict: Dict[str, ColumnDataSource] # ColumnDataSource 는 여러 객체의 정보가 들어있음
                frame_dict = {}
                ...
                for tracked_object_type_name, tracked_object_type in tracked_object_types.items(
                    ...
                    for tracked_object in tracked_objects.get_tracked_objects_of_type(
                        ...
                        agent_trajectory = translate_longitudinally(
                            object_box.center,
                            distance=object_box.length / 2 + 1)
                        trajectory_xs.append(
                            [object_box.center.x, agent_trajectory.x])
                        trajectory_ys.append(
                            [object_box.center.y, agent_trajectory.y])
...
class SimulationFigure:
    ...
    expert_trajectory_plot: Optional[
        GlyphRenderer] = None  # Expert trajectory plot
    legend_state: bool = False  # Legend states
    map_polygon_plots: Dict[str, GlyphRenderer] = field(
        default_factory=dict)  # Polygon plots for map layers
    map_line_plots: Dict[str, GlyphRenderer] = field(
        default_factory=dict)  # Line plots for map layers
    ...
    ego_state_trajectory_plot: Optional[
        EgoStateTrajectoryPlot] = None  # Ego state trajectory plot
    agent_state_plot: Optional[AgentStatePlot] = None  # Agent state plot
    agent_state_heading_plot: Optional[
        AgentStateHeadingPlot] = None  # Agent state heading plot

    # Optional simulation data
    lane_connectors: Optional[Dict[
        str, LaneConnector]] = None  # Lane connector id: lane connector

    # Dataclass
    glyph_names_from_checkbox_group: Optional[Dict[
        str, str]] = None  # Correct glyph names from checkbox groups
    ...
    def __post_init__(self) -> None:
        ...
        if self.ego_state_plot is None:
            self.ego_state_plot = EgoStatePlot(
    ...
    def is_rendering(self) -> bool:
        ...
        return any(plot.render_event.is_set() if plot.render_event else False
                   for plot in plots
                   if plot)
    ...
    def update_data_sources(self) -> None:
        ...
        # Update time_us
        self.time_us = [
            sample.ego_state.time_us for sample in self.simulation_history.data
        ]
        ...
        for plot in [
            ...
            if plot:
                t = threading.Thread(target=plot.update_data_sources,
                                     args=(self.simulation_history,),
                                     daemon=True)
                t.start()
    ...
    def render_mission_goal(self, mission_goal_state: StateSE2) -> None:
        ...
        """
        source = ColumnDataSource(
            dict(xs=[mission_goal_state.x],
                 ys=[mission_goal_state.y],
                 heading=[mission_goal_state.heading]))
    ...
    def render_expert_trajectory(
            self, expert_ego_trajectory_state: ColumnDataSource) -> None:
        """
        ...
        self.expert_trajectory_plot = self.figure.line(
            x="xs",
            y="ys",
            line_color=simulation_tile_trajectory_style["expert_ego"]
            ["line_color"],
            line_alpha=simulation_tile_trajectory_style["expert_ego"]
            ["line_alpha"],
            line_width=simulation_tile_trajectory_style["expert_ego"]
            ["line_width"],
    ...
    def get_glyph_name_from_checkbox_group(
            self, glyph_checkbox_group_name: str) -> str:
        """
        ...
        name = self.glyph_names_from_checkbox_group.get(
            glyph_checkbox_group_name, None)
        if not name:
            raise ValueError(
                f"{glyph_checkbox_group_name} is not a valid glyph name!")
        return name

    def _get_trajectory_glyph_to_update(
            self, glyph_name: str) -> List[Optional[GlyphRenderer]]:
        """
        ...
        if glyph_name == 'Expert Trajectory':
            return [
                self.expert_trajectory_plot
                if self.expert_trajectory_plot is not None else None
            ]
        elif glyph_name == 'Ego Trajectory':
            return [
                self.ego_state_trajectory_plot.plot
                if self.ego_state_trajectory_plot is not None else None
            ]
        elif glyph_name == 'Goal':
            return [self.mission_goal_plot]
        elif glyph_name == 'Traffic Light':
            return [
                self.traffic_light_plot.plot
                if self.traffic_light_plot is not None else None
            ]
        else:
            raise ValueError(f"{glyph_name} is not a valid trajectory name.")

    def _get_agent_glyph_to_update(
            self, glyph_name: str) -> List[Optional[GlyphRenderer]]:
        """
        ...
        :return A list of glyphs to be updated.
        """
        object_type_name = self.get_glyph_name_from_checkbox_group(
            glyph_checkbox_group_name=glyph_name)
        return [
            self.agent_state_plot.plots.get(object_type_name, None)
            if self.agent_state_plot is not None else None,
            self.agent_state_heading_plot.plots.get(object_type_name, None)
            if self.agent_state_heading_plot is not None else None,
        ]

    def update_glyphs_visibility(self,
        ...
        for glyph_name in glyph_names:
            if glyph_name == 'Ego':
                glyphs += [
                    self.ego_state_plot.plot
                    if self.ego_state_plot is not None else None
                ]
            elif glyph_name in [
                    'Expert Trajectory', 'Ego Trajectory', 'Goal',
                    'Traffic Light'
            ]:
                glyphs += self._get_trajectory_glyph_to_update(
                    glyph_name=glyph_name)
            elif glyph_name in [
                    'Vehicle', 'Pedestrian', 'Bicycle', 'Generic',
                    'Traffic Cone', 'Barrier', 'Czone Sign'
            ]:
                glyphs += self._get_agent_glyph_to_update(glyph_name=glyph_name)
            elif glyph_name in [
                    'Lane', 'Intersection', 'Stop Line', 'Crosswalk', 'Walkway',
                    'Carpark', 'RoadBlock'
            ]:
                map_polygon_name = self.get_glyph_name_from_checkbox_group(
                    glyph_checkbox_group_name=glyph_name)
                glyphs += [self.map_polygon_plots.get(map_polygon_name, None)]
            elif glyph_name in ['Lane Connector', 'Lane Line']:
                map_line_name = self.get_glyph_name_from_checkbox_group(
                    glyph_checkbox_group_name=glyph_name)
    ...
    def update_legend(self) -> None:
        ...
        agent_legends = [
            (category.capitalize(),
             [plot, self.agent_state_heading_plot.plots[category]])
        ...
        for map_polygon_layer in selected_map_polygon_layers:
            map_polygon_legend_items.append(
                (map_polygon_layer.capitalize(),
                 [self.map_polygon_plots[map_polygon_layer]]))
        selected_map_line_layers = [
            SemanticMapLayer.LANE.name, SemanticMapLayer.LANE_CONNECTOR.name
        ]
        map_line_legend_items = []
        for map_line_layer in selected_map_line_layers:
            map_line_legend_items.append(
                (map_line_layer.capitalize(),
                 [self.map_line_plots[map_line_layer]]))
        ...
        if self.traffic_light_plot and self.traffic_light_plot.plot is not None:
            legend_items.append(
                ("Traffic light", [self.traffic_light_plot.plot]))
