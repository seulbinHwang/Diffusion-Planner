...
from nuplan_extent.planning.nuboard.base.simulation_tile import SimulationTile
...
class ScenarioTimeSeriesData:
    ...
    time_series_selected_frames: Optional[
        List[int]]  # Time series selected frames


# Type for scenario metric score data type: {log name: {scenario name: metric score data}}
scenario_metric_score_dict_type = Dict[str, Dict[str,
                                                 List[ScenarioMetricScoreData]]]
...
class ScenarioTab(BaseTab):
    ...
    def __init__(
        ...
        self._scalar_log_name_select.on_change(
            "value", self._scalar_log_name_select_on_change)
        self._scalar_scenario_name_select = Select(
            name="scenario_scalar_name_select",
            css_classes=["scalar-scenario-name-select"],
        )
        self._scalar_scenario_name_select.js_on_change(
            "value", ScenarioTabUpdateWindowsSizeJSCode.get_js_code())
        self._scalar_scenario_name_select.on_change(
            "value", self._scalar_scenario_name_select_on_change)
        self._scenario_token_multi_choice = MultiChoice(
            **ScenarioTabScenarioTokenMultiChoiceConfig.get_config())
        self._scenario_token_multi_choice.on_change(
            "value", self._scenario_token_multi_choice_on_change)

        self._scenario_modal_query_btn = Button(
            **ScenarioTabModalQueryButtonConfig.get_config())
        self._scenario_modal_query_btn.js_on_click(
            ScenarioTabLoadingJSCode.get_js_code())
        self._scenario_modal_query_btn.on_click(
            self._scenario_modal_query_button_on_click)

        self.planner_checkbox_group.js_on_change(
            "active", ScenarioTabLoadingJSCode.get_js_code())
        ...
        self._simulation_tile_layout.js_on_change(
            "children", ScenarioTabLoadingEndJSCode.get_js_code())
        ...
        self._scenario_metric_score_data_figure_sizes = scenario_tab_style[
            'scenario_metric_score_figure_sizes']
        ...
        object_types = [
            'Ego', 'Vehicle', 'Pedestrian', 'Bicycle', 'Generic',
            'Traffic Cone', 'Barrier', 'Czone Sign'
        ]
        ...
        self._object_checkbox_group.on_change(
            'active', self._object_checkbox_group_active_on_change)
        trajectories = [
            'Expert Trajectory', 'Ego Trajectory', 'Goal', 'Traffic Light',
            'RoadBlock'
        ]
        ...
        self._traj_checkbox_group.on_change(
            'active', self._traj_checkbox_group_active_on_change)
        map_objects = [
            'Lane',
            'Intersection',
            'Stop Line',
        ...
        self._map_checkbox_group.on_change(
            'active', self._map_checkbox_group_active_on_change)
    ...
    def _traj_checkbox_group_active_on_change(self, attr: str, old: List[int],
                                              new: List[int]) -> None:
        """
        ...
        active_labels = [
            self._traj_checkbox_group.labels[index] for index in active_indices
        ]
        self._update_glyph_checkbox_group(glyph_names=active_labels)

    def _map_checkbox_group_active_on_change(self, attr: str, old: List[int],
                                             new: List[int]) -> None:
        """
        ...
        active_indices = list(set(old) - set(new)) + list(set(new) - set(old))
        active_labels = [
            self._map_checkbox_group.labels[index] for index in active_indices
        ]
        self._update_glyph_checkbox_group(glyph_names=active_labels)

    def _object_checkbox_group_active_on_change(self, attr: str, old: List[int],
                                                new: List[int]) -> None:
        """
        ...
        active_indices = list(set(old) - set(new)) + list(set(new) - set(old))
        active_labels = [
            self._object_checkbox_group.labels[index]
            for index in active_indices
        ]
        self._update_glyph_checkbox_group(glyph_names=active_labels)

    def file_paths_on_change(self, experiment_file_data: ExperimentFileData,
                             experiment_file_active_index: List[int]) -> None:
        """
    ...
    def _click_planner_checkbox_group(self, attr: Any) -> None:
        ...
        self._scenario_score_layout.children[0] = layout(
            scenario_metric_score_layout)

        # Filter time series data
        filtered_time_series_data: Dict[
            str, List[ScenarioTimeSeriesData]] = defaultdict(list)
        ...
        time_series_figure_data = self._render_time_series(
            aggregated_time_series_data=filtered_time_series_data)
        ...
        filtered_simulation_figures = [
            data for data in self._simulation_figure_data
            if data.planner_name in self.enable_planner_names
        ]
        if not filtered_simulation_figures:
            simulation_layouts = column(self._default_simulation_div)
            ego_expert_state_layouts = column(
                self._default_ego_expert_states_div)
        else:
            simulation_layouts = gridplot(
                [
                    simulation_figure.plot
                    for simulation_figure in filtered_simulation_figures
                ],
                ncols=self.get_plot_cols(
                    plot_width=self.simulation_figure_sizes[0],
                    offset_width=scenario_tab_style['col_offset_width']),
        ...
        self._ego_expert_states_layout.children[0] = layout(
            ego_expert_state_layouts)

    def _update_simulation_layouts(self) -> None:
        """Update simulation layouts."""
        self._simulation_tile_layout.children[0] = layout(
            self._simulation_plots)
    ...
    def _update_scenario_plot(self) -> None:
        ...
        # Render time series figure data
        time_series_figure_data = self._render_time_series(
            aggregated_time_series_data=self._time_series_data)
        # Render time series layout
        time_series_figures = self._render_scenario_metric_layout(
            figure_data=time_series_figure_data,
            default_div=self._default_time_series_div,
        ...
        ego_expert_state_layout = self._render_ego_expert_states(
            simulation_figure_data=self._simulation_figure_data)
        self._ego_expert_states_layout.children[0] = layout(
            ego_expert_state_layout)
    ...
    def _update_planner_names(self) -> None:
        ...
        selected_keys = [
            key for key in self.experiment_file_data.simulation_scenario_keys
        ...
        sorted_planner_names = sorted(
            list({key.planner_name for key in selected_keys}))
        self.planner_checkbox_group.labels = sorted_planner_names
        self.planner_checkbox_group.active = [
            index for index in range(len(sorted_planner_names))
        ]

    def _scalar_scenario_type_select_on_change(self, attr: str, old: str,
                                               new: str) -> None:
        """
        ...
        available_log_names = self.load_log_name(
            scenario_type=self._scalar_scenario_type_select.value)
    ...
    def _scalar_log_name_select_on_change(self, attr: str, old: str,
                                          new: str) -> None:
        """
        ...
        available_scenario_names = self.load_scenario_names(
            scenario_type=self._scalar_scenario_type_select.value,
            log_name=self._scalar_log_name_select.value)
        self._scalar_scenario_name_select.options = [""
                                                    ] + available_scenario_names
        self._scalar_scenario_name_select.value = ""

    def _scalar_scenario_name_select_on_change(self, attr: str, old: str,
                                               new: str) -> None:
        """
    ...
    def _scenario_token_multi_choice_on_change(self, attr: str, old: List[str],
                                               new: List[str]) -> None:
        """
    ...
    def _init_selection(self) -> None:
        ...
        if len(self._scalar_scenario_type_select.options) == 0:
            self._scalar_scenario_type_select.options = [
                ""
            ] + self.experiment_file_data.available_scenario_types

        if len(self._scalar_scenario_type_select.options) > 0:
            self._scalar_scenario_type_select.value = self._scalar_scenario_type_select.options[
                0]

        available_scenario_tokens = list(
            self._experiment_file_data.available_scenario_tokens.keys())
    ...
    def _render_scalar_figure(
        ...
        scenario_scalar_figure.title.text_font_size = scenario_tab_style[
            "time_series_figure_title_text_font_size"]
    ...
    def _update_aggregation_metric(self) -> scenario_metric_score_dict_type:
        ...
        """
        data: scenario_metric_score_dict_type = defaultdict(
            lambda: defaultdict(list))
        # Loop through all metric aggregators
        for index, metric_aggregator_dataframes in enumerate(
                self.experiment_file_data.metric_aggregator_dataframes):
            if index not in self._experiment_file_active_index:
                continue
            for file_index, (metric_aggregator_filename,
                             metric_aggregator_dataframe) in enumerate(
                        ...
                        if score is not None:
                            data[log_name][scenario_name].append(
                                ScenarioMetricScoreData(
                                    ...
                                    metric_aggregator_file_name=
                                    metric_aggregator_filename,
    ...
    def _aggregate_time_series_data(
            self) -> Dict[str, List[ScenarioTimeSeriesData]]:
        """
        Aggregate time series data.
        :return A dict of metric statistic names and their data.
        """
        aggregated_time_series_data: Dict[str,
                                          List[ScenarioTimeSeriesData]] = {}
        scenario_types = (tuple([self._scalar_scenario_type_select.value])
                          if self._scalar_scenario_type_select.value else None)
        log_names = tuple([self._scalar_log_name_select.value
                          ]) if self._scalar_log_name_select.value else None
        if not len(self._scalar_scenario_name_select.value):
            return aggregated_time_series_data
        for index, metric_statistics_dataframes in enumerate(
                self.experiment_file_data.metric_statistics_dataframes):
            if index not in self._experiment_file_active_index:
                continue
            ...
            for metric_statistics_dataframe in metric_statistics_dataframes:
                ...
                if metric_statistics_dataframe.metric_statistic_name not in aggregated_time_series_data:
                    aggregated_time_series_data[
                        metric_statistics_dataframe.metric_statistic_name] = []
                for planner_name in planner_names:
                    data_frame = metric_statistics_dataframe.query_scenarios(
                        scenario_names=tuple(
                            [str(self._scalar_scenario_name_select.value)]),
                    ...
                    time_series: pandas.DataFrame = data_frame[
                        time_series_headers]
                    ...
                    time_series_values: npt.NDArray[np.float64] = np.round(
                        np.asarray(
                            list(
                                chain.from_iterable(
                                    time_series[metric_statistics_dataframe.
                                                time_series_values_column]))),
                        4,
                    )
                    ...
                    time_series_unit = time_series[
                        metric_statistics_dataframe.
                        time_series_unit_column].iloc[0]
                    ...
                    aggregated_time_series_data[metric_statistics_dataframe.
                                                metric_statistic_name].append(
                                                    scenario_time_series_data)

        return aggregated_time_series_data

    def _render_time_series(
        self, aggregated_time_series_data: Dict[str,
                                                List[ScenarioTimeSeriesData]]
    ) -> Dict[str, Figure]:
        """
        ...
        for metric_statistic_name, scenario_time_series_data in aggregated_time_series_data.items(
            ...
            for data in scenario_time_series_data:
                ...
                if metric_statistic_name not in time_series_figures:
                    time_series_figures[
                        metric_statistic_name] = self._render_scalar_figure(
                ...
                color = self.experiment_file_data.file_path_colors[
                    data.experiment_index][data.planner_name]
                ...
                timestamp_frames = (
                    ...
                    if data.time_series_selected_frames is not None else list(
                        range(len(data.time_series_timestamps))))
                ...
                if data.time_series_selected_frames is not None:
                    time_series_figure.scatter(x="x",
                                               y="y",
                                               name=planner_name,
                                               color=color,
    ...
    def _render_scenario_metric_score_scatter(
        ...
        scenario_metric_score_data: Dict[str, List[ScenarioMetricScoreData]]
    ) -> None:
        """
        ...
        for metric_name, metric_score_data in scenario_metric_score_data.items(
            ...
            for index, score_data in enumerate(metric_score_data):
                experiment_name = self.get_file_path_last_name(
                    score_data.experiment_index)
                ...
                if data_source_index not in data_sources:
                    data_sources[
                        data_source_index] = ScenarioMetricScoreDataSource(
                            xs=[],
                            ys=[],
                            planners=[],
                            aggregators=[],
                            experiments=[],
                            fill_colors=[],
                            marker=self.get_scatter_sign(
                                score_data.metric_aggregator_file_index),
                            legend_label=legend_label,
                        )
                fill_color = self.experiment_file_data.file_path_colors[
                    score_data.experiment_index][score_data.planner_name]
                data_sources[data_source_index].xs.append(
                    score_data.metric_statistic_name)
                data_sources[data_source_index].ys.append(score_data.score)
                data_sources[data_source_index].planners.append(
                    score_data.planner_name)
                data_sources[data_source_index].aggregators.append(
                    score_data.metric_aggregator_file_name)
    ...
    def _render_scenario_metric_score(self) -> Dict[str, Figure]:
        ...
        """
        if (not self._scalar_log_name_select.value or
                not self._scalar_scenario_name_select.value or
                not self._scenario_metric_score_data):
            return {}
        selected_scenario_metric_score: List[
            ScenarioMetricScoreData] = self._scenario_metric_score_data[
                self._scalar_log_name_select.value][
                    self._scalar_scenario_name_select.value]
        ...
        number_of_figures = ceil(
            len(metric_statistic_names) / self._number_metrics_per_figure)
        ...
        for index in range(number_of_figures):
            ...
            selected_metric_names = metric_statistic_names[
            ...
            self._render_scenario_metric_score_scatter(
                scatter_figure=scenario_metric_score_figure,
                scenario_metric_score_data=metric_score_data)
            scenario_metric_score_figures[str(
                index)] = scenario_metric_score_figure
        return scenario_metric_score_figures

    def _render_grid_plot(self,
                          figures: Dict[str, Figure],
                          plot_width: int,
                          legend: bool = True) -> LayoutDOM:
        """
        Render a grid plot.
        ...
        for figure_name, figure_plot in figures.items():
            if legend:
                figure_plot.legend.label_text_font_size = scenario_tab_style[
                    "plot_legend_label_text_font_size"]
    ...
    def _render_scenario_metric_layout(self,
                                       figure_data: Dict[str, Figure],
                                       default_div: Div,
                                       plot_width: int,
                                       legend: bool = True) -> column:
        """
        ...
        grid_plot = self._render_grid_plot(figures=figure_data,
    ...
    def _render_simulations(self) -> column:
        ...
        selected_keys = [
            key for key in self.experiment_file_data.simulation_scenario_keys
            if key.scenario_type == self._scalar_scenario_type_select.value and
            key.log_name == self._scalar_log_name_select.value and
            key.scenario_name == self._scalar_scenario_name_select.value and
            key.nuboard_file_index in self._experiment_file_active_index
        ]
        if not selected_keys:
            ...
            hidden_glyph_names = [
                label for checkbox_group in [
                    self._object_checkbox_group, self._traj_checkbox_group,
                    self._map_checkbox_group
            ...
            simulation_figures = [
                data.plot for data in self._simulation_figure_data
            ]
            simulation_layouts = gridplot(
                simulation_figures,
                ncols=self.get_plot_cols(
    ...
    def _render_ego_expert_state_glyph(
        ...
        for plot_state_key in self.plot_state_keys:
            ...
            ego_expert_state_figure.yaxis.formatter = BasicTickFormatter(
        ...
        for planner_name, plot_states in ego_expert_plot_aggregated_states.items(
            ...
            for plot_state_key, plot_state_values in plot_states.items():
                ego_expert_state_figure = ego_expert_state_figures[
                    plot_state_key]
                data_source = ColumnDataSource(
                    dict(
    ...
    def _get_ego_expert_plot_color(self, planner_name: str,
                                   file_path_index: int,
                                   figure_planer_name: str) -> str:
        """
        ...
        return cast(
            ...
            if self.expert_planner_key in planner_name else
            self.experiment_file_data.file_path_colors[file_path_index]
            [figure_planer_name],
        )

    def _render_ego_expert_states(
            self, simulation_figure_data: List[SimulationData]) -> column:
        """
        ...
        for figure_data in simulation_figure_data:
            ...
            ego_expert_states = {
                expert_planner_name:
                    figure_data.simulation_figure.scenario.
                    get_expert_ego_trajectory(),
                ego_planner_name:
                    figure_data.simulation_figure.simulation_history.
                    extract_ego_state,
            }
            for planner_name, planner_states in ego_expert_states.items():
                # Get expert color
                ego_expert_plot_colors[
                    planner_name] = self._get_ego_expert_plot_color(
                        planner_name=planner_name,
                        figure_planer_name=figure_data.planner_name,
                        file_path_index=figure_data.simulation_figure.
                        file_path_index,
                    )
                ...
                for planner_state in planner_states:
                    for plot_state_key in self.plot_state_keys:
                        state_key_value = self._get_ego_expert_states(
                            state_key=plot_state_key, ego_state=planner_state)
                        ego_expert_plot_aggregated_states[planner_name][
                            plot_state_key].append(state_key_value)
