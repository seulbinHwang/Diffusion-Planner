...
from nuplan_extent.planning.nuboard.tabs.scenario_tab import ScenarioTab
...
class NuBoard:
    ...
    def __init__(
        ...
        if scenario_rendering_frame_rate_cap_hz < 1 or scenario_rendering_frame_rate_cap_hz > 60:
            raise ValueError(
                "scenario_rendering_frame_rate_cap_hz should be between 1 and 60"
            )
    ...
    def run(self) -> None:
        """Run nuBoard WebApp."""
        logger.info(
            f"Opening Bokeh application on http://localhost:{self._port_number}/"
        )
        logger.info(
            f"Async rendering is set to: {self._async_scenario_rendering}")
        ...
        server = Server(
            ...
            extra_patterns=[(r"/resource/(.*)", StaticFileHandler, {
    ...
    def main_page(self, doc: Document) -> None:
        ...
        """
        self._doc = doc
        template_path = Path(os.path.dirname(
        ...
        overview_tab = OverviewTab(doc=self._doc,
                                   experiment_file_data=experiment_file_data)
        histogram_tab = HistogramTab(doc=self._doc,
                                     experiment_file_data=experiment_file_data)
        ...
        self._doc.add_root(
            configuration_tab.experiment_file_path_checkbox_group)
