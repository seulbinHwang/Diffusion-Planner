...
from nuplan.planning.simulation.planner.ml_planner.transform_utils import _se2_vel_acc_to_ego_state


def vel_acc_to_waypoint(
    state: StateSE2,
    ...
    """
    oriented_box_ = OrientedBox.from_new_pose(ego_state.box, state)
    velocity_ = StateVector2D(*velocity)
    return Waypoint(time_point=TimePoint(int(timestamp * 1e6)),
                    oriented_box=oriented_box_,
                    velocity=velocity_)


def _get_ego_fixed_timesteps(state: EgoState, future_horizon: float,
...
def _get_velocity_and_acceleration(
    ...
    :param ego_poses: a list of the planned ego poses # 세계 좌표계 기준 poses
    ...
    current_agent_state = agent_history[-1]
    # state.metadata.timestamp_s

    # Past and current
    timesteps_past_current = [
        state.metadata.timestamp_s for state in agent_history
    ]
    ...
    ego_poses_current_planned: npt.NDArray[
        np.float32] = np.stack([current_agent_state.center.serialize()] +
...
def _get_absolute_waypoints_from_numpy_poses(
    ...
    :param poses: input relative poses # (80, 3) # 자기 자신의 중심 좌표계 기준
...
def _get_ego_velocity_and_acceleration(
    ego_poses: List[StateSE2], ego_history: Deque[EgoState],
...
def _get_absolute_agent_states_from_numpy_poses(
        poses: npt.NDArray[np.float32], set_coord_as_center: bool,
        ego_history: Deque[EgoState], timesteps: List[float]) -> List[EgoState]:
    """
    Converts an array of relative numpy poses to a list of absolute EgoState objects.
    ...
    if set_coord_as_center:
        origin_pose = ego_state.center
    else:
        origin_pose = ego_state.rear_axle
    absolute_states = relative_to_absolute_poses(origin_pose, relative_states)
    velocities, accelerations = _get_ego_velocity_and_acceleration(
...
def transform_ego_predictions_to_states(
    ...
    ego_state = ego_history[-1]
    # List[float] # [0.1, 0.2, ..., future_horizon] # length = 80
    timesteps = _get_ego_fixed_timesteps(ego_state, future_horizon,
                                         step_interval)
    states = _get_absolute_agent_states_from_numpy_poses(
        predicted_poses, set_coord_as_center, ego_history, timesteps)
...
def transform_predictions_to_states(
    predicted_poses: npt.NDArray[np.float32],  # 자기 자신의 중심 좌표계 기준 # (T, 3)
