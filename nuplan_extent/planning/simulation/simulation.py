...
class Simulation:
    ...
    def __init__(
        ...
        self._history = SimulationHistory(self._scenario.map_api,
                                          self._scenario.get_mission_goal())
    ...
    def is_simulation_running(self) -> bool:
        ...
        """
        return not self._time_controller.reached_end(
        ) and self._is_simulation_running
    ...
    def get_planner_input(self) -> PlannerInput:
        ...
        traffic_light_data = list(
            self._scenario.get_traffic_light_status_at_iteration(
                iteration.index))
        logger.debug(f"Executing {iteration.index}!")
        return PlannerInput(iteration=iteration,
    ...
    def propagate(self, trajectory: AbstractTrajectory) -> None:
        ...
        self._history.add_sample(
            SimulationHistorySample(iteration, ego_state, trajectory,
                                    observation, traffic_light_status))
