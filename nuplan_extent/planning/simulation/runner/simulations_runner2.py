...
from nuplan.planning.script.builders.metric_builder import \
    build_metrics_engines
from nuplan.planning.script.builders.observation_builder import \
    build_observations
...
def build_horizon_simulations(
    ...
    config = list(cfg.planner.values())[0]
    planner_cls: Type[AbstractPlanner] = _locate(config._target_)
    # Build planners
    if pre_built_planners is None and not planner_cls.requires_scenario:
        ...
        planners = build_planners(cfg.planner,
                                  scenario=None,
                                  cache=planners_cache)
    else:
        planners = pre_built_planners

    # Build a metric metadata file
    for scenario in tqdm(scenarios, total=len(scenarios)):
        ...
        for planner in planners:
            ...
            if "simulation_log_callback" in cfg.callback:
                stateful_callbacks.append(
                    instantiate(cfg.callback["simulation_log_callback"],
                                worker_pool=callbacks_worker))
            ...
            simulation = HorizonSimulation(
                simulation_setup=simulation_setup,
                callback=MultiCallback(stateful_callbacks + callbacks),
