...
def draw_neighbor_future_gt_3_dim(ax: plt.Axes, diff_token_to_future_gt_3_dim: Dict[str, Array],
    ...
    neighbor_future_gt_3_dim (max_agent_num, future_len, 3=[x,y,yaw])를
    흰색 'x' 마커로 그리고, 각 에이전트의 첫 점 근처에 인덱스(0..max_agent_num-1)를 흰색으로 표기.
...
def draw_ego(ax: plt.Axes, input_data: Dict[str, Any], draw_option):
    if draw_option.EGO_draw_ego_past:
        render_fast_collections.draw_ego_past_fast(ax, input_data.get("ego_agent_past").astype(np.float32), draw_option)
    if draw_option.EGO_draw_ego_agent_next_11_dim:
        render_fast_collections.draw_ego_agent_next_11_dim_fast(ax, input_data.get("ego_agent_next_11_dim").astype(np.float32), draw_option)
    if draw_option.EGO_draw_planner_future_11_dim:
        render_fast_collections.draw_planner_future_11_dim_fast(ax, input_data.get("planner_future_11_dim").astype(np.float32), draw_option)
    if draw_option.EGO_draw_ego_future_gt_11_dim:
        render_fast_collections.draw_ego_future_gt_11_dim_fast(ax, input_data.get("ego_future_gt_11_dim").astype(np.float32), draw_option)
...
def draw_neighbor_past_all(ax: plt.Axes, input_data: Dict[str, Any],
                           output_data: Optional[Dict[str, Any]],
                           draw_option, draw_token_list: Optional[List[str]]=None):
    neighbor_agents_past = input_data.get("neighbor_agents_past", None)
    if draw_option.NEI_draw_neighbor_past and (neighbor_agents_past is not None):
        draw_token_int_list = get_agent_idx_from_tokens(
            draw_token_list, input_data.get("neighbor_track_token", None))
        # ▶ 빠른 버전 호출
        render_fast_collections.draw_neighbor_past_fast(ax, neighbor_agents_past.astype(np.float32), draw_option, draw_token_int_list)
        ...
        if draw_option.NEI_draw_past_token and neighbor_track_token is not None:
            render_fast_collections.annotate_neighbor_indices_for_past_fast(ax, neighbor_track_token, neighbor_agents_past.astype(np.float32), draw_option)

    if draw_option.NEI_draw_neighbor_past_output:
        diff_token_to_np_history_wrt_ego = output_data.get("diff_token_to_np_history_wrt_ego", None)
        render_fast_collections.draw_neighbor_past_output_fast(ax, diff_token_to_np_history_wrt_ego, draw_option)
...
def draw_neighbor_future_all(ax: plt.Axes, input_data: Dict[str, Any],
                             output_data: Optional[Dict[str, Any]],
                             draw_option, draw_token_list: Optional[List[str]]=None):
    diff_token_to_future_gt_3_dim = input_data.get("diff_token_to_future_gt_3_dim", None)
    if draw_option.DIFF_draw_diff_future_gt_3_dim and diff_token_to_future_gt_3_dim is not None:
        render_fast_collections.draw_neighbor_future_gt_3_dim_fast(ax, diff_token_to_future_gt_3_dim, draw_option, draw_token_list)

    diff_token_to_np_gen_traj_wrt_ego = output_data.get("diff_token_to_np_gen_traj_wrt_ego", None)
    if draw_option.DIFF_draw_diff_future_gen_traj:
        render_fast_collections.draw_diff_future_gen_traj_fast(ax, diff_token_to_np_gen_traj_wrt_ego, draw_option, draw_token_list)

    diff_token_to_interp_np_traj_wrt_ego = output_data.get("diff_token_to_interp_np_traj_wrt_ego", None)
    diff_token_to_next_wp_wrt_ego = output_data.get("diff_token_to_next_wp_wrt_ego", None)
    if draw_option.DIFF_draw_diff_future_gen_refined_traj:
        render_fast_collections.draw_diff_future_gen_refined_traj_fast(
            ax, diff_token_to_interp_np_traj_wrt_ego, draw_option, diff_token_to_next_wp_wrt_ego, draw_token_list)
...
def draw_world_model_to_png(
    ...
    resize_figure_to_data_aspect(fig, bounds, target_long_side_px=2000)
