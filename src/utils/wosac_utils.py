...
def get_scenario_rollouts(
    ...
    for i_scenario in range(n_scenario):
            ...
            for i_agent in range(len(agent_id[i_scenario])):
                simulated_trajectories.append(
                    sim_agents_submission_pb2.SimulatedTrajectory(
                        center_x=pred_traj[i_scenario][i_agent, i_rollout, :,
                                                       0],
                        center_y=pred_traj[i_scenario][i_agent, i_rollout, :,
                                                       1],
        ...
        _str_scenario_id = "".join(
            [chr(x) for x in scenario_id[i_scenario] if x > 0])
...
def get_scenario_id_int_tensor(scenario_id: List[str],
