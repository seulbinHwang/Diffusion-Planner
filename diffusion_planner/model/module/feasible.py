...
class FeasibleProjector(nn.Module):
        ...
        if self.use_feasible_dl:
            ...
            self.trunk_compressor = nn.Sequential(
                ...
                nn.Linear(hidden_dim,
                          3 * self._Dc),  #: hidden_dim → 64 (중간 폭 축소)
                nn.GELU(),
    ...
    def _apply_S2_accel_alpha_limits_ste(
        ...
        """(S2) 차/자전거에만 '가속도/각가속도' 제약을 적용합니다.

        변경점:
            - 차/자전거(nonholonomic): 개선안 B 적용
              - (vx, vy) 자체의 벡터 차이를 제한하지 않고,
                '부호 있는 속도 크기(s_signed)'의 변화만 제한합니다.
              - 그 뒤, 원래 (vx, vy)가 가리키던 방향(옆으로 미끄러지는 정도)은 유지한 채로
                (vx, vy)를 다시 만듭니다.
            - 보행자(holonomic): S2를 아예 적용하지 않습니다.
              (즉, 선형가속 제한/각가속 제한 모두 건너뜁니다)

        Args:
            vx_b_prev, vy_b_prev, omega_prev:
                이전 구간의 제어(필터 적용 후 값). shape: (B,Pnn)
            vx_b, vy_b, omega:
                현재 구간의 제어(이미 S0/S1 적용된 값이 들어오는 흐름). shape: (B,Pnn)
            a_max:
                종 방향 가감속 상한으로 사용할 값. shape: (B,Pnn)
            alpha_max:
                요각가속도(omega 변화) 상한. shape: (B,Pnn)
            dt:
                시간 간격(초)
            eta, eps:
                STE 밴드 폭 / 수치 안정 값
            is_nonholonomic:
                차/자전거면 True, 보행자면 False. shape: (B,Pnn)

        Returns:
            (vx_b_out, vy_b_out, omega_out): 모두 (B,Pnn)
        """
        nonh = self._to_bool_mask(is_nonholonomic).to(device=vx_b.device)
        if not bool(nonh.any()):
            # 전부 보행자면 그대로 반환
            return vx_b, vy_b, omega

        # --- (1) 선형: 개선안 B (부호 있는 속도 크기 변화만 제한) ---
        s_cur, c_cur, u_cur = self._compute_signed_speed_and_slip_components(
            vx_b=vx_b, vy_b=vy_b, eps=eps
        )  # (B,Pnn) each
        s_prev, _, _ = self._compute_signed_speed_and_slip_components(
            vx_b=vx_b_prev, vy_b=vy_b_prev, eps=eps
        )  # (B,Pnn)

        ds = s_cur - s_prev  # (B,Pnn)
        ds_limit = (a_max.to(dtype=vx_b.dtype, device=vx_b.device) * float(
            dt))  # (B,Pnn)
        ds_ste = self._ste_scalar_clip(ds, ds_limit, eta=float(eta),
                                       eps=float(eps))  # (B,Pnn)

        s_new = s_prev + ds_ste  # (B,Pnn)

        vx_new = s_new * c_cur  # (B,Pnn)
        vy_new = s_new.abs() * u_cur  # (B,Pnn)

        # --- (2) 각가속: omega 변화 제한(차/자전거에만) ---
        dw = omega - omega_prev  # (B,Pnn)
        dw_limit = (alpha_max.to(dtype=omega.dtype,
                                 device=omega.device) * float(dt))  # (B,Pnn)
        dw_ste = self._ste_scalar_clip(dw, dw_limit, eta=float(eta),
                                       eps=float(eps))  # (B,Pnn)
        omega_new = omega_prev + dw_ste  # (B,Pnn)

        # --- (3) 보행자는 S2 미적용 ---
        vx_out = torch.where(nonh, vx_new, vx_b)
        vy_out = torch.where(nonh, vy_new, vy_b)
        omega_out = torch.where(nonh, omega_new, omega)
    ...
    def _apply_S2_accel_alpha_limits_ste_batch(
        ...
        eps: float,
        is_nonholonomic: torch.Tensor,  # (B,Pnn) bool
        target_current_control: Optional[torch.Tensor] = None,        # (B,Pnn,3) or None
        target_current_control_valid: Optional[torch.Tensor] = None,  # (B,Pnn) or None
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """(S2, 배치) 차/자전거에만 개선안 B + 각가속 제한을 적용합니다.

        - 차/자전거(nonholonomic):
            1) 각 시점 k에서 '부호 있는 속도 크기(s_signed[k])'의 증가/감소만 제한
            2) 그 시점의 (vx, vy)가 가리키던 방향(옆으로 미끄러지는 정도)은 유지한 채로
               (vx, vy)를 다시 만듦
            3) omega는 기존처럼 '증분(dw)'만 제한 (각가속 제한)

        - 보행자(holonomic):
            - S2 자체를 적용하지 않음 (선형가속/각가속 모두 제한 X)

        k=0 처리:
            - target_current_control이 없으면: k>=1만 제한(기존 동작)
            - target_current_control이 있으면:
                valid=True인 행만 k=0도 제한(기존 동작)

        Returns:
            vx_out, vy_out, omega_out: 모두 (B,Pnn,T)
        """
        if vx_b.dim() != 3 or vy_b.dim() != 3 or omega.dim() != 3:
            raise ValueError("vx_b/vy_b/omega는 (B,Pnn,T) 3D여야 합니다.")
        if vx_b.shape != vy_b.shape or vx_b.shape != omega.shape:
            raise ValueError("vx/vy/omega shape이 서로 다릅니다.")

        B, Pnn, T = vx_b.shape
        if T <= 0:
            return vx_b, vy_b, omega

        nonh_bp = self._to_bool_mask(is_nonholonomic).to(device=vx_b.device)  # (B,Pnn)
        if not bool(nonh_bp.any()):
            # 전부 보행자면 그대로 반환
            return vx_b, vy_b, omega

        # -------------------------
        # (1) 현재 시퀀스에서 방향 성분(c,u) + 부호 있는 속도 크기(s_signed) 계산
        # -------------------------
        s_signed, c, u = self._compute_signed_speed_and_slip_components(
            vx_b=vx_b, vy_b=vy_b, eps=eps
        )  # (B,Pnn,T), (B,Pnn,T), (B,Pnn,T)

        # -------------------------
        # (2) "prev 없음" 버전(기존 동작): k>=1만 증분 제한
        # -------------------------
        a_lim_bp = a_max.to(dtype=vx_b.dtype, device=vx_b.device) * float(dt)        # (B,Pnn)
        alpha_lim_bp = alpha_max.to(dtype=omega.dtype, device=omega.device) * float(dt)  # (B,Pnn)

        if T == 1:
            s_old = s_signed
            omega_old = omega
        else:
            ds = s_signed[..., 1:] - s_signed[..., :-1]  # (B,Pnn,T-1)
            ds_lim = a_lim_bp.unsqueeze(-1).expand(B, Pnn, T - 1)  # (B,Pnn,T-1)
            ds_ste = self._ste_scalar_clip(ds, ds_lim, eta=float(eta), eps=float(eps))  # (B,Pnn,T-1)
            ds_prefix = torch.cumsum(ds_ste, dim=2)  # (B,Pnn,T-1)
            s0 = s_signed[..., :1]  # (B,Pnn,1)
            s_old = torch.cat([s0, s0 + ds_prefix], dim=2)  # (B,Pnn,T)

            dw = omega[..., 1:] - omega[..., :-1]  # (B,Pnn,T-1)
            dw_lim = alpha_lim_bp.unsqueeze(-1).expand(B, Pnn, T - 1)  # (B,Pnn,T-1)
            dw_ste = self._ste_scalar_clip(dw, dw_lim, eta=float(eta), eps=float(eps))  # (B,Pnn,T-1)
            dw_prefix = torch.cumsum(dw_ste, dim=2)  # (B,Pnn,T-1)
            w0 = omega[..., :1]  # (B,Pnn,1)
            omega_old = torch.cat([w0, w0 + dw_prefix], dim=2)  # (B,Pnn,T)

        # -------------------------
        # (3) prev 있는 버전: valid=True인 행만 k=0도 포함
        # -------------------------
        if target_current_control is None:
            s_sel = s_old
            omega_sel = omega_old
        else:
            if target_current_control.dim() != 3 or int(target_current_control.shape[-1]) != 3:
                raise ValueError("target_current_control은 (B,Pnn,3) 이어야 합니다.")
            if tuple(target_current_control.shape[:2]) != (int(B), int(Pnn)):
                raise ValueError("target_current_control의 (B,Pnn)이 입력과 다릅니다.")

            if target_current_control_valid is None:
                use_prev_bp = torch.ones((int(B), int(Pnn)), device=vx_b.device, dtype=torch.bool)
            else:
                if target_current_control_valid.dim() != 2 or tuple(target_current_control_valid.shape) != (int(B), int(Pnn)):
                    raise ValueError("target_current_control_valid는 (B,Pnn)이어야 합니다.")
                use_prev_bp = self._to_bool_mask(target_current_control_valid).to(device=vx_b.device)

            # 모두 False면 prev 없는 동작과 동일
            if not bool(use_prev_bp.any()):
                s_sel = s_old
                omega_sel = omega_old
            else:
                tc = target_current_control.to(device=vx_b.device, dtype=vx_b.dtype)  # (B,Pnn,3)
                tc = tc.masked_fill(~use_prev_bp.unsqueeze(-1), 0.0)
                ...
                s_prev0, _, _ = self._compute_signed_speed_and_slip_components(
                    vx_b=vx_prev0, vy_b=vy_prev0, eps=eps
                )  # (B,Pnn)

                # k=0 증분 + 나머지 증분
                ds0 = s_signed[..., :1] - s_prev0.unsqueeze(-1)      # (B,Pnn,1)
                ds_rest = s_signed[..., 1:] - s_signed[..., :-1]     # (B,Pnn,T-1)
                ds_all = torch.cat([ds0, ds_rest], dim=2)            # (B,Pnn,T)

                ds_lim_all = a_lim_bp.unsqueeze(-1).expand(B, Pnn, T)  # (B,Pnn,T)
                ds_ste_all = self._ste_scalar_clip(ds_all, ds_lim_all, eta=float(eta), eps=float(eps))
                ds_prefix_all = torch.cumsum(ds_ste_all, dim=2)        # (B,Pnn,T)
                s_new = s_prev0.unsqueeze(-1) + ds_prefix_all          # (B,Pnn,T)

                dw0 = omega[..., :1] - w_prev0.unsqueeze(-1)           # (B,Pnn,1)
                dw_rest = omega[..., 1:] - omega[..., :-1]             # (B,Pnn,T-1)
                dw_all = torch.cat([dw0, dw_rest], dim=2)              # (B,Pnn,T)

                dw_lim_all = alpha_lim_bp.unsqueeze(-1).expand(B, Pnn, T)  # (B,Pnn,T)
                dw_ste_all = self._ste_scalar_clip(dw_all, dw_lim_all, eta=float(eta), eps=float(eps))
                dw_prefix_all = torch.cumsum(dw_ste_all, dim=2)            # (B,Pnn,T)
                omega_new = w_prev0.unsqueeze(-1) + dw_prefix_all          # (B,Pnn,T)

                m_prev = use_prev_bp.unsqueeze(-1)  # (B,Pnn,1)
                s_sel = torch.where(m_prev, s_new, s_old)
                omega_sel = torch.where(m_prev, omega_new, omega_old)

        # -------------------------
        # (4) 선택된 s_sel로 (vx,vy) 복원 (슬립각 유지)
        # -------------------------
        vx_new = s_sel * c               # (B,Pnn,T)
        vy_new = s_sel.abs() * u         # (B,Pnn,T)

        # -------------------------
        # (5) 보행자는 S2 미적용: 최종 마스킹
        # -------------------------
        m_nonh = nonh_bp.unsqueeze(-1)  # (B,Pnn,1)
        vx_out = torch.where(m_nonh, vx_new, vx_b)
        vy_out = torch.where(m_nonh, vy_new, vy_b)
        omega_out = torch.where(m_nonh, omega_sel, omega)
    ...
    def _filter_and_integrate_sequential(
        ...
        # (B,Pnn,3) or None (unnorm)
        target_current_control_valid: Optional[torch.Tensor] = None,
        # (B,Pnn) or (B,Pnn==1) or None
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """sequential 적분 + (선택적) 제약 필터 적용.

        k=0 처리 규칙
        - prev_valid=True인 행: k=0에서도 S2를 적용
        - prev_valid=False인 행: k=0에서는 S2를 적용하지 않음(기존 동작)
        """
        ...
        apply_filter_any: bool = True if (filter_active_idx is None) else (int(
            filter_active_idx.numel()) > 0)
        ...
        # prev 사용 마스크 (B,Pnn)
        if target_current_control is None:
            use_prev_bp = torch.zeros((int(B), int(Pnn)),
                                      device=device,
                                      dtype=torch.bool)
            ...
            if target_current_control.dim() != 3 or int(
                    target_current_control.shape[-1]) != 3:
                raise ValueError("target_current_control은 (B,Pnn,3)이어야 합니다.")
            if target_current_control_valid is None:
                use_prev_bp = torch.ones((int(B), int(Pnn)),
                                         device=device,
                                         dtype=torch.bool)
            else:
                if target_current_control_valid.dim() != 2:
                    raise ValueError(
                        "target_current_control_valid는 (B,Pnn) 2D여야 합니다.")
                use_prev_bp = self._to_bool_mask(
                    target_current_control_valid).to(device=device)

            tc = target_current_control.to(device=device,
                                           dtype=dtype)  # (B,Pnn,3)
            tc = tc.masked_fill(~use_prev_bp.unsqueeze(-1), 0.0)
        ...
        for k in range(future_len):
            vx_b_k_raw = vx_b_raw[..., k]
            vy_b_k_raw = vy_b_raw[..., k]
            w_k_raw = omega_raw[..., k]
            ...
            if self.use_feasible_filter and apply_filter_any:
                if k == 0:
                    # k=0: S2 적용/미적용을 둘 다 계산하고, prev_valid로 선택
                    vx_s2, vy_s2, w_s2 = self._apply_constraints_step(
                        vx_b_prev=vx_b_prev,
                        vy_b_prev=vy_b_prev,
                    ...
                    vx_n, vy_n, w_n = self._apply_constraints_step(
                        vx_b_prev=vx_b_prev,
                        vy_b_prev=vy_b_prev,
                        omega_prev=omega_prev,
                        vx_b_k=vx_b_k_raw,
                    ...
                    vx_f = torch.where(use_prev_bp, vx_s2, vx_n)
                    vy_f = torch.where(use_prev_bp, vy_s2, vy_n)
                    w_f = torch.where(use_prev_bp, w_s2, w_n)
                else:
                ...
                w_k = torch.where(apply_filter_mask_bp, w_f, w_k_raw)
            ...
            omega_list.append(w_k)

            x_k, y_k, cos_yaw_k, sin_yaw_k = x_k1, y_k1, cos_k1, sin_k1
            vx_b_prev, vy_b_prev, omega_prev = vx_b_k, vy_b_k, w_k
        ...
        mask_node = near_cur_future_valid.to(torch.bool)
        mask_interval = mask_node[..., :-1] & mask_node[..., 1:]
    ...
    def _gather_active_target_current_control_for_filter_and_integrate(
            self,
            target_current_control: torch.Tensor,  # (B, Pnn, 3)
            active_indices: torch.Tensor,  # (N_active,)
    ) -> torch.Tensor:
        """active 슬롯만 모아 (N_active, 1, 3) 형태로 압축합니다.

        Args:
            target_current_control (torch.Tensor):
                shape: (B, Pnn, 3)
                현재 시점 제어. [v_x^b, v_y^b, w]
            active_indices (torch.Tensor):
                shape: (N_active,)
                (B*Pnn)으로 펼쳤을 때의 active row 인덱스.

        Returns:
            torch.Tensor:
                shape: (N_active, 1, 3)
                active 슬롯에 해당하는 현재 시점 제어만 모은 텐서.
        """
        if target_current_control.dim() != 3 or int(
                target_current_control.shape[-1]) != 3:
            raise ValueError(
                "[_gather_active_target_current_control_for_filter_and_integrate] "
                "target_current_control은 (B,Pnn,3) 이어야 합니다. "
                f"got shape={tuple(target_current_control.shape)}")

        B, Pnn, _ = target_current_control.shape
        B_Pnn = int(B * Pnn)

        ctrl_flat = target_current_control.reshape(B_Pnn, 3)  # (B*Pnn, 3)
        idx = active_indices.to(device=ctrl_flat.device,
    ...
    def _build_filter_mask_flat_bp_from_filter_active_idx(
        *,
        ...
        idx = filter_active_idx.to(device=device,
                                   dtype=torch.long).view(-1)  # (B_filter,)
        if int(idx.numel()) == 0:
            filter_mask_b = torch.zeros((B,), device=device,
                                        dtype=torch.bool)  # (B,)
        else:
        ...
        # (B,) -> (B,Pnn) -> (B*Pnn,)
        filter_mask_flat_bp = filter_mask_b.unsqueeze(1).expand(
            int(B), int(Pnn)).reshape(-1)  # (B*Pnn,)
    ...
    def _convert_filter_active_idx_to_active_subset_indices(
        ...
        *,
        ...
        selected_in_active = filter_and_active_mask_flat.index_select(
            0, active_indices.to(device=device,
                                 dtype=torch.long))  # (N_active,) bool

        # 최종: active subset 기준 인덱스 목록 (0..N_active-1)
        filter_active_idx_active = selected_in_active.nonzero(
            as_tuple=False).squeeze(-1).to(
                device=device, dtype=torch.long)  # (N_filter_active,)
    ...
    def _build_apply_filter_mask_bp_from_filter_active_idx(
        *,
        ...
        """
        if filter_active_idx is None:
            return torch.ones((int(B), int(Pnn)),
                              device=device,
                              dtype=torch.bool)
        ...
        idx = filter_active_idx.to(device=device,
                                   dtype=torch.long).view(-1)  # (B_filter,)
        if int(idx.numel()) == 0:
            mask_b = torch.zeros((int(B),), device=device,
                                 dtype=torch.bool)  # (B,)
        else:
            idx_min = int(idx.min().item())
            idx_max = int(idx.max().item())
    ...
    def _scatter_active_subset_for_control_sequence(
        ...
        *,
        ...
        """
        B_Pnn = int(B * Pnn)
        out_ctrl_flat = torch.zeros((B_Pnn, int(T), 3),
                                    device=device,
                                    dtype=dtype)  # (B*Pnn,T,3)
        ...
        if int(active_indices.numel()) > 0:
            idx = active_indices.to(device=device,
                                    dtype=torch.long)  # (N_active,)
            src = control_sequence_active.squeeze(1).to(
                device=device, dtype=dtype)  # (N_active,T,3)
    ...
    def _gather_active_target_current_control_valid_for_filter_and_integrate(
            self,
            target_current_control_valid: torch.Tensor,  # (B,Pnn) bool/0-1
            active_indices: torch.Tensor,  # (N_active,)
    ) -> torch.Tensor:
        """active 슬롯에 해당하는 prev_valid만 모읍니다.

        Args:
            target_current_control_valid (torch.Tensor):
        ...
        Returns:
            torch.Tensor:
                active 슬롯만 모은 prev_valid.
                shape: (N_active, 1)  (Pnn을 1로 압축했기 때문)
        """
        if target_current_control_valid.dim() != 2:
            raise ValueError("target_current_control_valid는 (B,Pnn) 2D여야 합니다. "
                             f"got {tuple(target_current_control_valid.shape)}")
        mask_bool = self._to_bool_mask(
            target_current_control_valid)  # (B,Pnn) bool

        B, Pnn = mask_bool.shape
        mask_flat = mask_bool.reshape(int(B * Pnn))  # (B*Pnn,)
        idx = active_indices.to(device=mask_flat.device, dtype=torch.long)
    ...
    def filter_and_integrate(
        ...
        if target_current_control_valid is not None and target_current_control is None:
            raise ValueError(
                "target_current_control_valid가 주어졌으면 target_current_control도 함께 주어져야 합니다."
            )
        ...
        if target_current_control is not None:
            ...
            if tuple(target_current_control.shape[:2]) != (int(B), int(Pnn)):
                raise ValueError(
                    ...
                    f"expected ({int(B)},{int(Pnn)}), got {tuple(target_current_control.shape[:2])}"
                )
        ...
        if int(active_indices.numel()) == 0:
            z4 = unnorm_cur_future_seg_body_control.new_zeros((B, Pnn, T, 4))
            z3 = unnorm_cur_future_seg_body_control.new_zeros((B, Pnn, T, 3))
            return z4, z3, z3
        ...
        target_current_control_active: Optional[torch.Tensor] = None
        target_current_control_valid_active: Optional[torch.Tensor] = None

        if target_current_control is not None:
            target_current_control_active = self._gather_active_target_current_control_for_filter_and_integrate(
                target_current_control=target_current_control.to(device=device,
                                                                 dtype=dtype),
                active_indices=active_indices,
            )  # (N_active,1,3)
        ...
        if not self.use_batch_integration:
            traj_active, diff_active, ctrl_active = self._filter_and_integrate_sequential(
                ...
                target_current_control_valid=target_current_control_valid_active,
            )
        else:
        ...
        unnorm_control_sequence = self._scatter_active_subset_for_control_sequence(
            control_sequence_active=ctrl_active,
            active_indices=active_indices,
            B=int(B),
            Pnn=int(Pnn),
            T=int(T),
    ...
    def _filter_and_integrate_batch(
        ...
        vx_b_after, vy_b_after, omega_after = vx_b_raw, vy_b_raw, omega_raw
        ...
        if self.use_feasible_filter and apply_filter_any:
            ...
            mask_bp1 = apply_filter_mask_bp.unsqueeze(-1)
        ...
        unnorm_control_sequence = torch.stack(
            [vx_b_after, vy_b_after, omega_after], dim=-1)
        mask_node = near_cur_future_valid.to(torch.bool)
        mask_interval = mask_node[..., :-1] & mask_node[..., 1:]
        unnorm_control_sequence = unnorm_control_sequence.masked_fill(
            ~mask_interval.unsqueeze(-1), 0.0)
    ...
    def _compute_signed_speed_and_slip_components(
        ...
        """부호 있는 속도 크기와, '방향 성분(슬립각 유지용)'을 계산합니다.

        여기서 하는 일:
            1) 속도 크기 s = sqrt(vx^2 + vy^2)
            2) 전진/후진을 구분하기 위해 s에 부호를 붙인 값 s_signed를 만듭니다.
               - vx >= 0 이면 +, vx < 0 이면 -
               - vx==0인 경우는 +로 취급합니다. (0이면 부호가 0이 되어 버리는 문제 방지)
            3) 현재 (vx, vy)가 가리키는 '방향'을 다음 두 값으로 저장합니다.
               - c = |vx| / (s + eps)   (0~1)
               - u =  vy  / (s + eps)   (-1~1)
               이후 속도 크기만 바꿀 때,
                   vx = s_signed * c
                   vy = |s_signed| * u
               로 복원하면, '방향(옆으로 미끄러지는 정도)'이 유지됩니다.

        Args:
            vx_b: (B,Pnn) 또는 (B,Pnn,T) 바디 x방향 속도
            vy_b: (B,Pnn) 또는 (B,Pnn,T) 바디 y방향 속도
            eps: 0으로 나눔/수치 문제를 막기 위한 작은 값

        Returns:
            s_signed: vx 부호를 반영한 속도 크기. shape는 vx_b와 동일
            c: |vx|/(s+eps). shape는 vx_b와 동일
            u: vy/(s+eps). shape는 vx_b와 동일
        """
        s = torch.sqrt(vx_b * vx_b + vy_b * vy_b + float(eps))  # (...,)

        # vx==0이면 +로 취급 (torch.sign(0)=0 문제 방지)
        one = torch.ones_like(vx_b)
        sigma = torch.where(vx_b >= 0.0, one, -one)  # (...,)
