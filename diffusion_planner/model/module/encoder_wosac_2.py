...
class Encoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.route_order_drop_prob: float = getattr(config, "route_order_drop_prob", 0.5)
        ...
        # ✅ LaneSummaryTokenPooler 완전 미사용: lane 토큰은 절대 요약하지 않음
        # 외부 코드가 attribute 존재를 기대할 수도 있으면 아래처럼 None으로만 명시해도 됩니다.
    ...
    def iter_encoder_global_parameters(self) -> Iterator[nn.Parameter]:
        ...
        글로벌 인코더는 여러 객체 토큰을 섞어 장면 표현을 만드는 부분입니다.
        (✅ lane 요약 모듈은 더 이상 사용하지 않습니다.)
        """
        global_modules = [
            self.fusion,
        ]
    ...
    def _build_empty_road_safety_tokens(
        ...
        """road-safety 입력이 아예 없을 때 사용할 '빈 토큰(길이 0)'을 만듭니다.

        Args:
            batch_size (int): 배치 크기 B.
            ref_encoding (torch.Tensor): dtype/device 기준 텐서. 예: encoding_lanes (B, N, H)
            ref_pos (torch.Tensor): dtype/device 기준 텐서. 예: lane_pos (B, N, 9)

        Returns:
            Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
                - encoding_road_safety: (B, 0, H)
                - road_safety_mask:     (B, 0)  True=pad(무효) 규칙을 그대로 따르지만, 길이 0이라 비어있음
                - road_safety_pos:      (B, 0, 9)
        """
        ...
        # (B, 0)
        road_safety_mask: torch.Tensor = torch.zeros(
            (B, 0),
            device=ref_encoding.device,
            dtype=torch.bool,
        )
        ...
        # DDP unused param 방지용: 값은 그대로(빈 텐서)지만 그래프만 0계수로 연결
        touch_f32: torch.Tensor = torch.zeros(
            (), device=ref_encoding.device, dtype=torch.float32
        )
...
class RoadSafetyFusionEncoder(nn.Module):
    ...
    '입력 개수 그대로' 토큰으로 변환해 반환하는 인코더입니다.

    변경점(핵심)
    ----------
    - 더 이상 num_seeds로 요약(압축)하지 않습니다.
    - 출력 토큰 개수는 입력 요소 개수의 합(Ns+Nc+Nb+Nd+Ne)입니다.
    - road_safety_pos는 각 요소의 꼭지점 좌표 평균으로 x,y를 만들고,
      방향(cos,sin)은 (1,0)으로 고정합니다.

    반환
    ----------
    - encoding_road_safety: (B, N_total, hidden_dim)
    - road_safety_mask:     (B, N_total)  True=pad(무효)
    - road_safety_pos:      (B, N_total, 9) = [x,y,cos,sin] + type_onehot(5)
    ...
    @staticmethod
    def _principal_axis_dir(points_xy_f32: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
        ...
        return torch.stack([torch.cos(theta), torch.sin(theta)], dim=-1)  # (M, 2)
    ...
    def _compute_geometry_features(
        ...
        """요소(폴리곤/폴리라인)의 점 집합에서 기하 요약을 만듭니다.
        ...
        # 임베딩 feature는 기존대로 principal axis 사용(표현력/호환성 유지)
        dir_for_feat = RoadSafetyFusionEncoder._principal_axis_dir(pts)  # (M, 2)
        ...
        numeric_feat = torch.cat([center, dir_for_feat, extent, seglen, closed_flag], dim=-1)  # (M, 8)

        # ✅ pos는 "꼭지점 평균"만 사용, 방향은 (1,0) 고정
        pos_dir = torch.zeros((pts.shape[0], 2), device=pts.device, dtype=torch.float32)  # (M, 2)
        if int(pos_dir.shape[0]) > 0:
            pos_dir[:, 0] = 1.0
        pos4 = torch.cat([center, pos_dir], dim=-1)             # (M, 4)
    ...
    def _encode_category(
        ...
        if num_valid > 0:
            ...
            numeric_feat_f32, pos4_f32 = self._compute_geometry_features(points_valid, closed=closed)  # (M,8),(M,4)
            ...
            emb_valid = emb_valid + self.category_scale.to(emb_valid.dtype) * cat_emb
            ...
            if road_edge_type is not None:
                ...
                edge_type_valid = road_edge_type.reshape(B * N, 3)[valid_flat].to(
                    device=device, dtype=emb_valid.dtype
                )  # (M,3)
    ...
    def _concat_parts(
        ...
        return torch.cat(embs, dim=1), torch.cat(masks, dim=1), torch.cat(pos4s, dim=1)

    @staticmethod
    def _build_pos9_from_pos4(pos4: torch.Tensor) -> torch.Tensor:
        """pos4([x,y,cos,sin])에 type_onehot(5)을 붙여 (B,N,9)을 만듭니다."""
        B, N, _ = pos4.shape
        token_type = torch.zeros((B, N, 5), device=pos4.device, dtype=pos4.dtype)
        token_type[:, :, 4] = 1.0  # road_safety
        return torch.cat([pos4, token_type], dim=-1)               # (B,N,9)
    ...
    def forward(
        ...
        """Road-safety 입력을 '입력 개수 그대로' 토큰으로 변환합니다.

        Returns:
            encoding_road_safety: (B, N_total, hidden_dim)
            road_safety_mask:     (B, N_total) True=pad
            road_safety_pos:      (B, N_total, 9)
        """
        ...
        # 유효 요소가 하나도 없으면: 파라미터 touch로 DDP unused 방지
        total_valid: int = int((~elements_mask).sum().item())
        if total_valid == 0:
            elements_emb = self._touch_self_parameters(elements_emb)

        # 출력 정규화/드롭 후, pad는 0으로 고정
        elements_emb = self.out_drop(self.out_norm(elements_emb))
        elements_emb = elements_emb.masked_fill(elements_mask.unsqueeze(-1), 0.0)

        road_safety_pos = self._build_pos9_from_pos4(elements_pos4)
        road_safety_pos = road_safety_pos.masked_fill(elements_mask.unsqueeze(-1), 0.0)

        return elements_emb, elements_mask, road_safety_pos
