...
class ShardDistributedSampler(Sampler[int]):
    ...
    ----
    - 에폭마다 shard 순서만 셔플
    - shard를 rank마다 겹치지 않게 분배
    - rank 안에서 shard를 worker마다 겹치지 않게 분배
    - DataLoader의 worker 배치 분배(round-robin) 구조를 이용해,
      실제로 worker가 '자기 shard'만 처리하는 순서를 만들어 줍니다.
    """
        ...
        *,
        ...
        Args:
            ...
            seed:
                shard 셔플 시드. shape: ()
            num_workers:
                DataLoader num_workers. shape: ()
            per_rank_batch_size:
                rank당 배치 크기. 예: 256. shape: ()
        """
        ...
        self.shards_per_rank = self.shard_count // self.num_replicas  # 726/6=121
        self.num_samples = self.shards_per_rank * self.shard_size      # 121*2048=247,808

        self.batches_per_shard = self.shard_size // self.per_rank_batch_size  # 2048/256=8
    ...
    def _shuffle_shard_ids_for_epoch(self) -> List[int]:
        """에폭 seed로 shard id(0..shard_count-1)만 섞는다."""
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        rank_shards = shard_ids[start:end]  # length=121

        # 3) worker별 round-robin 분배 (겹치지 않게)
        ...
        # 4) "배치 단위"로 worker0→worker1→... 순서로 내보내서,
        #    DataLoader worker round-robin과 맞물리게 만든다.
